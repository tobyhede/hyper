...
**Handle**:
React Flow's attachment point on a node, where an edge meets it or an author begins or ends drawing one. Graph authoring presents four Graph-independent handles on a Thing, one on each side, coloured as the Active Graph; the side is interaction geometry and is not authored. A renderer may use other invisible attachment geometry to keep existing Edges legible, but that remains rendering and never becomes a rule the model obeys.
