...
A Map holds one or many Graphs; each Graph has a title and may have a colour so Graphs can be told apart when the Map shows several at once; a new Graph is given the colour that stands farthest from those its Map's other Graphs already carry, and keeps it until an author recolours it. An empty Graph is fully authored and may be active, but cannot be presented until it has an Edge. A Space with no Map has no Graphs; creating a Map also creates its initial empty Active Graph in the same Edit.
