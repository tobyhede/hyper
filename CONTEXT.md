...
_Avoid_: expand as a *second verb* beside Opening (a Card is Opened, and Expanded is the state that leaves it in), preview, popup, modal, dialog, drill-down, view mode and edit mode for content Opening (which has one surface, the Card). An Alias's metadata-authoring dialog is the explicit exception: an Alias has no content front of its own to expand, and that dialog does not make it Expanded.
