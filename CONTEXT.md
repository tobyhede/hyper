...
**Handle**:
React Flow's attachment point on a node, where an edge meets it or an author begins or ends drawing one. A Thing has four Graph-independent handles, one on each side; the side is interaction geometry and is not authored. They are **anchors** before they are affordances: every Thing carries all four, an Edge attaches to whichever of them faces the other Thing, and that choice is made while the Edge is drawn from where the two Things are at that moment. Only where an author may take hold of one is withheld — coloured as the Active Graph, revealed on hover or Selection, and absent from a Thing the canvas does not author. None of it is a rule the model obeys.
