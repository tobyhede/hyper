...
A Hyper-specific visual or semantic composite is encouraged when it adds reusable Hyper meaning. `CanvasResource` composed from shared Card primitives is such a component. A second locally implemented Dialog for a styling difference is not.
