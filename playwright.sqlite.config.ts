...
export default defineConfig({
  testDir: './test/e2e',
  testMatch: 'sqlite-persistence.spec.ts',
  fullyParallel: false,
  workers: 1,
  // This project runs in CI's `sqlite` job, so it owes the same `.only` guard
  ...
  // browser and round-trips the SQLite file between them. Playwright's 30s
  // default is a cold-start away from failing on timing rather than on
  // durability.
  timeout: 120_000,
  reporter: 'list',
  ...
  projects: [{ name: 'sqlite', use: { ...devices['Desktop Chrome'], trace: 'retain-on-failure' } }],
});
