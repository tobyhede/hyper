...
Opening an Alias is the same Layout-owned Open operation every Card uses. The Open Alias keeps its own Title and renders the Target Card's content read-only through ADR 0009's single-hop resolution. A Markdown Target supplies rendered Markdown without a source editor. To author content or content configuration, the author opens the Target Card itself.

When the proposed Space Card kind in ADR 0068 is accepted and built, the same capability rule applies: an Alias may render whatever content the Target Space Card owns, but cannot change that Card's Space View, Graph or other content configuration. ADR 0070 does not settle ADR 0068's provisional renderer or gestures.

The rendering reuses the Target kind's existing content renderer under a read-only capability rather than creating Alias-specific rendering. Close and Resize remain normal Card operations and persist the containing Layout. A future **Jump to Target Card**, and any shortcut from there into the Target's ordinary Edit interaction, use durable Card navigation under ADR 0069; they do not author the Target in place through the Alias.
