...
`CommitResult`'s two failure arms carry a code and no `message`
(`.scratch/error-feedback-pattern/issues/01`). What a `protocol` failure broke
is typed context instead of prose: its required `fault` is a closed
`ProtocolFault` naming the broken expectation with its offending Space ids,
statuses or thrown value, for a diagnostic and never for copy. The stored
seam's `RepositoryCommitResult` keeps its `rejected.message`, which the HTTP
host sends as `problem.detail` and the browser's adapter does not carry across.
