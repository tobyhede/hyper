...
import { createHandler } from '../support/e2e-http-runtime';
...
const startRuntime = async (catalog: 'fixture' | 'empty') => {
  const server = await startHttpServer(await createHandler({ catalog }));
  servers.push(server);
...
describe('e2e HTTP runtime', () => {
  it('imports the abstract-layout fixture into each fresh runtime', async () => {
    const first = await startRuntime('fixture');
    const second = await startRuntime('fixture');
