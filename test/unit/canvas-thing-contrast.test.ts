import { readFileSync } from 'node:fs';
import { describe, expect, it } from 'vitest';

/**
 * The canvas Thing draws on its own cream paper, warmer and lighter than the
 * chrome's, so its text roles are separate tokens rather than the chrome's
 * `--foreground` and `--muted-foreground`. Separate means they can drift: a
 * colour picked against chrome paper can land on cream and stay green through
 * every other check — `canvas-thing.spec.ts` asserts that the refusal message
 * says the right words, never that it can be read.
 *
 * The threshold is WCAG 2.2 AA for body text (1.4.3): 4.5:1. Both Thing faces
 * are checked, because a state change swaps one for the other under text whose
 * own colour does not change.
 */

const theme = readFileSync(new URL('../../packages/app/src/tailwind.css', import.meta.url), 'utf8');
const thingCss = readFileSync(
  new URL('../../packages/ui/src/canvas-thing.css', import.meta.url),
  'utf8',
);

const AA_BODY_TEXT = 4.5;

const tokens = new Map(
  [...`${theme}\n${thingCss}`.matchAll(/^\s*(--[\w-]+):\s*([^;]+);/gm)].map((match) => [
    match[1]!,
    match[2]!.trim(),
  ]),
);

/** Follows a `var(--a)` chain to the literal colour it bottoms out in. */
function resolve(value: string, seen = new Set<string>()): string {
  const reference = /^var\((--[\w-]+)\)$/u.exec(value.trim());
  if (reference === null) return value.trim();
  const name = reference[1]!;
  if (seen.has(name)) throw new Error(`cyclic custom property: ${name}`);
  const next = tokens.get(name);
  if (next === undefined) throw new Error(`undefined custom property: ${name}`);
  return resolve(next, new Set(seen).add(name));
}

function channel(component: number): number {
  const c = component / 255;
  return c <= 0.03928 ? c / 12.92 : ((c + 0.055) / 1.055) ** 2.4;
}

function luminance(hex: string): number {
  const digits = /^#([0-9a-f]{6})$/iu.exec(hex);
  if (digits === null) throw new Error(`expected a six-digit hex colour, got ${hex}`);
  const value = digits[1]!;
  const [r, g, b] = [0, 2, 4].map((i) => channel(Number.parseInt(value.slice(i, i + 2), 16)));
  return 0.2126 * r! + 0.7152 * g! + 0.0722 * b!;
}

function contrast(foreground: string, background: string): number {
  const a = luminance(foreground);
  const b = luminance(background);
  const lighter = Math.max(a, b);
  const darker = Math.min(a, b);
  return (lighter + 0.05) / (darker + 0.05);
}

/** Every text role the Thing draws on one of its own faces. */
const TEXT_ON_THING = [
  { role: 'title', token: '--canvas-thing-title-color' },
  { role: 'Alias Target line', token: '--canvas-thing-muted-color' },
  { role: 'title refusal message', token: '--canvas-thing-error-color' },
] as const;

const FACES = [
  { state: 'rest', token: '--canvas-thing-face-rest' },
  { state: 'active', token: '--canvas-thing-face-active' },
] as const;

describe('canvas Thing text on its own faces', () => {
  const pairs = TEXT_ON_THING.flatMap((text) =>
    FACES.map((face) => ({
      name: `${text.role} on the ${face.state} face`,
      ink: text.token,
      face: face.token,
    })),
  );

  it.each(pairs)('reads at AA: $name', ({ ink, face }) => {
    const ratio = contrast(resolve(`var(${ink})`), resolve(`var(${face})`));
    expect(ratio).toBeGreaterThanOrEqual(AA_BODY_TEXT);
  });
});

describe('canvas Thing face paint', () => {
  it('stops at the inner edge of its border', () => {
    const thingRule = /^\.canvas-thing \{(?<declarations>[\s\S]*?)^\}/mu.exec(thingCss);

    expect(thingRule?.groups?.['declarations']).toMatch(/background-clip:\s*padding-box;/u);
  });
});
