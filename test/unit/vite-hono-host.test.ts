...
describe('Vite Hono host', () => {
  ...
  it('mints working-load initialization from the identity source it was composed with', async () => {
    // The host's `newId` reaches both collaborators it composes that mint, not just
    // the root address. A stored mapless Space is durably initialized on
    // first working load (ADR 0079), and that Map and Graph are the ones
    // this composition named — not the ambient generator's, which is what
    // `createSpaceHttpApp`'s own default would have supplied.
    const ids = [MINTED_MAP_ID, MINTED_GRAPH_ID];
    const spaceRepository = new MemorySpaceRepository(
      [{ snapshot, revision: 0n, exportedRevision: null }],
      SPACE_ID,
    );
  ...
  it('answers a removed canvas identity as not found', async () => {
    ...
    const response = await fetch(
      `${host.url}/spaces/${encodeCompactUuid(SPACE_ID)}/maps/${encodeCompactUuid(MAP_ID)}`,
    );
  ...
  it('resolves HEAD like GET while sending no product response body', async () => {
    ...
    const missing = await fetch(`${host.url}/spaces/${encodeCompactUuid(RESOURCE_ID)}`, {
      method: 'HEAD',
    });
