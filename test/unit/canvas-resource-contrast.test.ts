...
/**
 * The canvas Resource draws on its own cream paper, warmer and lighter than the
 ...
 * every other check — `canvas-resource.spec.ts` asserts that the refusal message
 * says the right words, never that it can be read.
 *
 * The threshold is WCAG 2.2 AA for body text (1.4.3): 4.5:1. Both Resource faces
...
const resourceCss = readFileSync(
  new URL('../../packages/ui/src/canvas-resource.css', import.meta.url),
  'utf8',
);
...
const tokens = new Map(
  [...`${theme}\n${resourceCss}`.matchAll(/^\s*(--[\w-]+):\s*([^;]+);/gm)].map((match) => [
    match[1]!,
    match[2]!.trim(),
  ]),
);
...
/** Every text role the Resource draws on one of its own faces. */
const TEXT_ON_RESOURCE = [
  { role: 'title', token: '--canvas-resource-title-color' },
  { role: 'Reference Resource Target line', token: '--canvas-resource-muted-color' },
  { role: 'title refusal message', token: '--canvas-resource-error-color' },
] as const;

const FACES = [
  { state: 'rest', token: '--canvas-resource-face-rest' },
  { state: 'active', token: '--canvas-resource-face-active' },
] as const;

describe('canvas Resource text on its own faces', () => {
  const pairs = TEXT_ON_RESOURCE.flatMap((text) =>
    FACES.map((face) => ({
...
describe('canvas Resource face paint', () => {
  it('stops at the inner edge of its border', () => {
    const resourceRule = /^\.canvas-resource \{(?<declarations>[\s\S]*?)^\}/mu.exec(resourceCss);

    expect(resourceRule?.groups?.['declarations']).toMatch(/background-clip:\s*padding-box;/u);
  });
});
