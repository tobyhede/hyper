...
import { importSpaceBatch } from '../../src/import/import-space';
...
export type E2eHttpRuntimeOptions = {
  ...
  /** A directory of Space directories, imported together so Space Cards resolve. */
  | { catalog: 'batch'; directory: string }
);
...
const importCatalog = (repository: E2eMemorySpaceRepository, options: E2eHttpRuntimeOptions) => {
  ...
  if (options.catalog === 'batch') return importSpaceBatch(options.directory, repository);
