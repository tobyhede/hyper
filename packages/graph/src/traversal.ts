import type { ResourceId, Graph, GraphEdge } from '@project/core';

/**
 * Reading a Graph as something to traverse (ADR 0024, 0027).
 *
 * Presenting traverses a Graph: at a Resource, the presenter follows one of its
 * outgoing Edges. These are the reads that traversal supports — what a Resource's moves are,
 ...
 *
 * **A line is not a special case.** A graph where every resource has one outgoing
...
/** The edges leaving a resource, in the order the author wrote them. A resource's moves. */
export function outgoingEdges(graph: Graph, resourceId: ResourceId): GraphEdge[] {
  return graph.edges.filter((edge) => edge.from === resourceId);
}

/**
 * The Resources a traversal can start from: those an Edge leaves but none arrives at, in
 ...
 */
export function graphEntryResources(graph: Graph): ResourceId[] {
 ...
 *
 * **A resource nothing arrives at wins**, and this rule stays first. It makes the
 ...
 * self-edge rule 2 *derives* the start — one resource, one edge, nothing to choose
 * — while for a multi-resource cycle it *picks* one, and any resource on the loop would
 ...
 * loop. Whether a traversal can reach every Resource of a disconnected Graph is a
 ...
 * a change here.
 */
export function graphStartResource(graph: Graph): ResourceId | undefined {
  return graphEntryResources(graph)[0] ?? graph.edges[0]?.from;
}
