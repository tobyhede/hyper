import type { ThingId, Graph, GraphEdge } from '@project/core';

/**
 * Reading a Graph as something to traverse (ADR 0024, 0027).
 *
 * Presenting traverses a Graph: at a Thing, the presenter follows one of its
 * outgoing Edges. These are the reads that traversal supports — what a Thing's moves are,
 ...
 *
 * **A line is not a special case.** A graph where every thing has one outgoing
...
/** The edges leaving a thing, in the order the author wrote them. A thing's moves. */
export function outgoingEdges(graph: Graph, thingId: ThingId): GraphEdge[] {
  return graph.edges.filter((edge) => edge.from === thingId);
}

/**
 * The Things a traversal can start from: those an Edge leaves but none arrives at, in
 ...
 */
export function graphEntryThings(graph: Graph): ThingId[] {
  const arrivedAt = new Set(graph.edges.map((edge) => edge.to));
  const entries: ThingId[] = [];
 ...
 *
 * **A thing nothing arrives at wins**, and this rule stays first. It makes the
 ...
 * `a → b` stores the `b` edge first, and starting there would skip `a` — a thing
 ...
 *
 * **Otherwise the first edge's `from`.** Every thing of a fully cyclic graph is
 ...
 * self-edge rule 2 *derives* the start — one thing, one edge, nothing to choose
 * — while for a multi-thing cycle it *picks* one, and any thing on the loop would
 ...
 * loop. Whether a traversal can reach every Thing of a disconnected Graph is a
 ...
 * a change here.
 */
export function graphStartThing(graph: Graph): ThingId | undefined {
  return graphEntryThings(graph)[0] ?? graph.edges[0]?.from;
}
