...
import { outgoingEdges, graphStartResource } from '../src/index';
// Internal to the package: `graphStartResource` is the offered way in.
import { graphEntryResources } from '../src/traversal';
...
describe('outgoingEdges', () => {
  ...
  it('gives a resource on a line exactly one — the degenerate fork', () => {
    expect(
  ...
  it('gives a resource the graph does not touch none', () => {
    expect(outgoingEdges(diamond, uuid('00000000-0000-4000-8000-000000000098'))).toEqual([]);
  });
});

describe('graphEntryResources', () => {
  it('finds the resource nothing arrives at', () => {
    expect(graphEntryResources(diamond)).toEqual(['00000000-0000-4000-8000-000000000002']);
  });
  ...
  it('answers none for a fully cyclic Graph, which is the honest answer', () => {
    // "A resource nothing arrives at" is a true question about the structure, and a
    // loop has none. `graphStartResource` decides what to do about that; making this
    // report a resource would make the word a lie.
    expect(
      graphEntryResources(
        graph([
          [
...
describe('graphStartResource', () => {
  it('starts traversal at the first entry', () => {
    expect(graphStartResource(diamond)).toBe('00000000-0000-4000-8000-000000000002');
  });

  it('is undefined only for a graph with no edges, which the schema forbids', () => {
    expect(graphStartResource(graph([]))).toBeUndefined();
  });

  it('starts a self-connected Graph at its only resource', () => {
    // The first gesture authoring ships: connecting a resource to itself. Nothing
    // arrives at nothing here — the resource arrives at itself — so rule 1 has no
  ...
  it('picks the first edge’s source for a cycle no resource enters', () => {
    // Three resources, all arrived at. Rule 2 *picks* here rather than deriving:
  ...
  it('does not depend on which resource the first edge happens to mention', () => {
