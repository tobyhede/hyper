...
/**
 * The catalogue ground these panes are shown on, matching the canvas Thing's own
...
const markdown: Extract<Thing, { kind: 'markdown' }> = {
  id: MARKDOWN_ID,
  title: 'Architecture notes',
  kind: 'markdown',
  body: `## Placement

Placement is authored, not computed. A Diagram owns an explicitly positioned subset of Things.
...
Opening authors one Thing in place. Presenting traverses the Active Graph.`,
};
    ...
    >
      {created === null ? (
        <NewAlias
          ...
          // tells it to mint `Thing N` — the Target's own Title is never copied
          // (ADR 0083 refines ADR 0046). So the unnamed completion this stands
          // in for reports the minted name.
          onCreate={(target, title) =>
            setCreated(`Created ${title === '' ? 'the minted Thing N' : title} on ${target}.`)
          }
