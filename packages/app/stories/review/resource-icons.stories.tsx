 ...
 *   Resource    sticky-note  a note, not a filed document
 *   Graph   route        a path from a start pin to an end pin
 *   Map  layout-grid  placements on a plane
 *   Reference Resource   a corner badge on whichever of those it is a Reference Resource of
 *
 ...
 * chrome where a Map is authored placement on a plane (ADR 0014 — placement
 * is authored, not computed).
 *
 * **A Reference Resource is not a fifth glyph, and that is the load-bearing decision.** A
 * single Reference Resource mark can say *that* a Resource refers elsewhere but never *what it
 * refers to* — and a Space Resource can be a Reference Resource's Target as much as a Markdown
 * Resource can, so the two would draw identically. The kind of the Resource on the
 ...
 * `canvas-resource.css` keeps the Resource and only changes `border-style` to dotted.
 *
 ...
 * not on `CanvasResourceFront` at all — so every Reference Resource still draws over the
 ...
 * 20px, which is roughly a Resource Front.
 */
import type { Story } from '@ladle/react';
import './resource-icons.css';

export default { title: 'Review/Resource Icons' };
 ...
 * a liberty: `canvas-resource.css` already draws a Reference Resource with `border-style:
...
interface Candidate {
  ...
  /** Drawn with the Reference Resource's own dotted stroke rather than lucide's solid one. */
  readonly dash?: string | undefined;
}

/**
 * Per kind, because the choice is made per kind — and then judged as a trio,
 * because a Resource's kind is only ever read next to the other two.
 */
...
const SPACE: readonly Candidate[] = [
  ...
  {
    name: 'sticky-notes',
    note: 'Notes plural. Pairs with a notepad Markdown glyph and says "the Resources in here" without borrowing Map or Graph.',
  },
  { name: 'file-stack', note: 'The same claim, filed rather than pinned. Heavier at 14px.' },
  { name: 'box', note: 'A container with contents. No collision with Map or Graph.' },
  { name: 'boxes', note: 'Many contained shapes. Busier; blurs at 14px.' },
  { name: 'square-stack', note: 'Resources stacked in one place. Close to a duplicate mark.' },
  ...
  { name: 'frame', note: 'A canvas. Collides with PanelsTopLeft — reads as a Map.' },
];

/**
 * **A Reference Resource is not a fourth kind; it is another Resource seen from elsewhere.** So
 * the base glyph does not change and a decoration is added to it — which is
 * what the domain says (ADR 0070: an Open Reference Resource resolves the immutable Target's
 * Markdown into the same content front a Markdown Resource draws) and what the
 * canvas already does (`canvas-resource.css` keeps the Resource and only changes
 * `border-style` to dotted).
 *
 * **What settles it is that a Space Resource can be a Reference Resource's Target too.** A
 * single Reference Resource glyph can say *that* a Resource refers elsewhere but never *what it
 * refers to*, so a Reference Resource of a Space and a Reference to a Markdown Resource would draw
 * identically — and the kind of the Resource on the canvas is exactly what the
 ...
 * on the Resource glyph and on the Space glyph.
 *
 ...
 * says Markdown Resource or Space Resource, and the badge says this one is a view of
 * another.
 */
...
const REFERENCE_OPTIONS: readonly ReferenceOption[] = [
  ...
  },
  {
    ...
    note: 'A solid mark at the corner: this Resource points at another. Survives 14px because a filled badge is a shape, not a line weight, and it reads on the Space glyph as well as the Resource one.',
  },
  {
...
/**
 * Graph and Map are no longer fixed points.
 *
 * They were the two constraints the Resource kinds had to dodge, and both are now
 * themselves in question — which loosens the Resource columns as much as it opens
 ...
 * with branches and joins, over Resources a Map has already placed.
 * `PanelsTopLeft` is a web page: header, sidebar, content. That is a **document
 * chrome**, and a Map here is authored placement on a plane (ADR 0014 —
...
const GRAPH: readonly Candidate[] = [
  ...
  {
    name: 'route',
    note: 'A path from a start pin to an end pin, doubling back. The closest resource to "traverse this Graph", and Present is what a Graph is for.',
  },
  {
    name: 'git-graph',
...
const MAP: readonly Candidate[] = [
  { name: 'panels-top-left', note: 'In the tree. Header, sidebar, content — a web page.' },
  {
    name: 'group',
    note: 'Selection corners around two placed rectangles. "These Resources, positioned together" is what a Map owns.',
  },
  {
  ...
  {
    name: 'axis-3d',
    note: 'Coordinates. Honest about stored x/y, and cold — it names the mechanism, not the resource.',
  },
  {
    name: 'layout-grid',
 ...
 * somewhere a Resource kind is also drawn — the Dock's clusters, a Resource rail, an
...
const SPENT: readonly {
  ...
  { name: 'plus', icon: 'PlusIcon', means: 'Create Resource, New Map, New Graph', rulesOut: '—' },
  { name: 'trash-2', icon: 'DeleteIcon', means: 'Delete', rulesOut: '—' },
  { name: 'pencil', icon: 'EditIcon', means: 'Edit a Resource', rulesOut: '—' },
  {
    name: 'maximize-2',
    icon: 'OpenResourceIcon',
    means: 'Open a Resource',
    rulesOut: 'square-arrow-out',
  },
 ...
 *
 * Four and not three: Graph and Map stopped being fixed constraints once
 * `Network` was read as a tree and `PanelsTopLeft` as a web page, so the whole
 * entity vocabulary is one choice. The Reference Resource is not a fifth entry — it is the
 * Resource entry with a decoration, which is the point.
 */
interface EntitySet {
  readonly title: string;
  readonly claim: string;
  readonly space: GlyphName;
  readonly resource: GlyphName;
  readonly graph: GlyphName;
  readonly map: GlyphName;
}
 ...
 * and the two Reference Resources — a Reference Resource of a Resource and a Reference Resource of a Space — because
 * the decoration only earns its place if those two are told apart at a glance,
 * which is the distinction a single Reference Resource glyph could never make.
 */
const ENTITY_ROWS = [
  { kind: 'space', label: 'Space', example: 'Rendering' },
  { kind: 'resource', label: 'Resource', example: 'Why authored placement beats a layout engine' },
  { kind: 'graph', label: 'Graph', example: 'Long' },
  { kind: 'map', label: 'Map', example: 'Collection 1' },
  { kind: 'resource', label: 'Reference Resource', example: 'Strategy overview', reference: true },
  { kind: 'space', label: 'Reference Resource', example: 'Rendering overview', reference: true },
...
/**
 * The Resource glyph with a Reference Resource decoration on it.
 *
 * The badge is drawn at the bottom-right over a knockout disc painted in the
 * chrome's own paper, so it sits on the Resource rather than tangling with the
...
function ReferenceMark({
  ...
  /** The glyph being decorated — a Markdown Resource's or a Space Resource's. */
...
export const Default: Story = () => (
  <div className="icons">
    <p className="icons__lede">
      ...
      Resource Front), in lucide&rsquo;s own geometry and stroke weight.
    </p>
    ...
    <section className="icons__collisions">
      <h2 className="icons__heading">
        A Reference Resource decorates the glyph it is a Reference Resource of — Resource, then
        Space
      </h2>
    ...
    <div className="icons__columns">
      <CandidateColumn heading="Resource — shortlisted: sticky-note" candidates={MARKDOWN} />
      ...
      <CandidateColumn heading="Map — shortlisted: layout-grid" candidates={MAP} />
    </div>
