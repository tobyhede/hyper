 ...
 * `canvas-thing.css` keeps the Thing and only changes `border-style` to dotted.
 *
 ...
 * 20px, which is roughly a Thing Front.
 */
import type { Story } from '@ladle/react';
import './thing-icons.css';

export default { title: 'Review/Thing Icons' };
 ...
 * because a Thing's kind is only ever read next to the other two.
 */
...
const SPACE: readonly Candidate[] = [
  ...
  {
    name: 'sticky-notes',
    note: 'Notes plural. Pairs with a notepad Markdown glyph and says "the Things in here" without borrowing Diagram or Graph.',
  },
  ...
  { name: 'square-stack', note: 'Things stacked in one place. Close to a duplicate mark.' },
 ...
 * on the Thing glyph and on the Space glyph.
 *
 ...
 *
 * They were the two constraints the Thing kinds had to dodge, and both are now
 * themselves in question — which loosens the Thing columns as much as it opens
 ...
 * with branches and joins, over Things a Diagram has already placed.
...
const DIAGRAM: readonly Candidate[] = [
  ...
  {
    name: 'group',
    note: 'Selection corners around two placed rectangles. "These Things, positioned together" is what a Diagram owns.',
  },
  {
 ...
 * somewhere a Thing kind is also drawn — the Dock's clusters, a Thing rail, an
...
const SPENT: readonly {
  ...
  { name: 'plus', icon: 'PlusIcon', means: 'Create Thing, New Diagram, New Graph', rulesOut: '—' },
  { name: 'trash-2', icon: 'DeleteIcon', means: 'Delete', rulesOut: '—' },
  { name: 'pencil', icon: 'EditIcon', means: 'Edit a Thing', rulesOut: '—' },
  {
    name: 'maximize-2',
    icon: 'OpenThingIcon',
    means: 'Open a Thing',
...
export const Default: Story = () => (
  <div className="icons">
    <p className="icons__lede">
      ...
      Thing Front), in lucide&rsquo;s own geometry and stroke weight.
    </p>
    ...
    <div className="icons__columns">
      <CandidateColumn heading="Thing — shortlisted: sticky-note" candidates={MARKDOWN} />
