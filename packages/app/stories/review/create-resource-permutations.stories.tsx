/**
 * THROWAWAY UX PROTOTYPE — what Create Resource costs in the Resources cluster.
 *
 ...
 *
 * **The complaint, stated as a count.** The Dock drew `[▢ Resources ⌄][+]` and the
 ...
 * Resource, and it is the one kind that needs no second decision — so the most
 ...
 *
 *   markdown  `addResource()`              — one activation completed an Edit
 *   reference     `resourceCreation.open()`    — opened a modal pane (a Target was required)
 *   space     `resourceCreation.open()`    — opened a modal pane (a Space was required)
 *
 * ADR 0089 has since deleted `resourceCreation` outright: every kind now completes
 * on activation, and Reference Resource creation left this cluster for a `Create
 * Reference` row on the Resource it targets. The table above is the asymmetry this
 ...
 * and `icons.tsx` documents `PlusIcon` as **"Create a Markdown Resource"** then as
 * now. The history is real too: `AddResourceControl` was a split button, deleted by
 ...
 *
 * **Nothing here creates a Resource.** Every press is recorded in the log instead,
 ...
 * `ResourcesTrigger` and the real `command-dock.css`, so the options are judged on
...
import { RESOURCES_TRIGGER } from '#components/command-dock-triggers';
...
import { ResourcesTrigger } from '#components/CommandDock';
import './create-resource-peer-row.css';

export default { title: 'Review/Create Resource' };

/** The three kinds, in the order `CreateMenu` once listed them. */
const RESOURCE_KINDS = ['markdown', 'space', 'reference'] as const;
 ...
 * has since made every kind complete on activation and moved Reference Resource
 * creation onto the Resource it targets — these outcomes are the review's record,
 * not current behaviour.
 */
const KIND_OUTCOME = {
  markdown: 'Edit completed — the Resource is on the canvas',
...
/** What a reviewer reads in place of a created Resource. */
function PressLog({
  log,
  onReset,
}: {
    ...
    <div className="w-full max-w-[38rem] rounded-md border bg-background/95 p-3 shadow-sm">
      ...
      {log.length === 0 ? (
        <p className="font-mono text-[11px] text-muted-foreground">
          Nothing yet — try creating a Markdown Resource in each strip and watch the counts.
        </p>
      ) : (
...
function PrototypeBanner({ children }: { readonly children: string }) {
  return (
    <div className="bg-amber-400 px-3 py-1 text-center text-xs font-semibold text-amber-950">
      PROTOTYPE — {children} Nothing here creates a Resource.
    </div>
  );
}
 ...
 * costs next to `Resources ⌄`, and a control judged out of its cluster is judged
 * against nothing.
 */
function DockStrip({
  vertical = false,
  ...
  /** An arrangement to try on the Resources cluster, in place of the dock's grid. */
  readonly clusterClassName?: string;
  /** A column width to try, where the question is how much room an option needs. */
  readonly width?: number;
  /** Draw the three identity clusters above Resources, as a ruler for the columns. */
    ...
    <div className="w-fit">
      ...
      >
        ...
        <ToolbarGroup
          ...
          className={cn('command-dock__cluster', clusterClassName)}
        >
          <ToolbarButton variant="ghost" {...RESOURCES_TRIGGER}>
            <ResourcesTrigger />
          </ToolbarButton>
          {children}
        </ToolbarGroup>
        ...
        <DropdownMenuGroup>
          ...
          {RESOURCE_KINDS.map((kind) => (
            <DropdownMenuItem
              key={kind}
              className="gap-2"
              onClick={() => record(`${resourceKindName(kind)} — ${KIND_OUTCOME[kind]}`)}
            >
 ...
 * on its own says *what a Resource is*, not *make one*. The same three glyphs
 * already appear in the Resources list as row markers and on the canvas as the
 * Resource's own kind, so three of them in a command slot are the first place in
...
function PeerCreate({ record }: { readonly record: (line: string) => void }) {
  return (
    ...
    // positional logic, so it nests inside the Resources cluster and leaves the
    ...
    // give the peers a row of their own (`create-resource-peer-row.css`).
    //
    ...
    <ToolbarGroup aria-label="Create a Resource" className="cdp-peer-row">
      {RESOURCE_KINDS.map((kind) => (
        <ToolbarButton
          key={kind}
          variant="ghost"
 ...
 * a Reference Resource **already** carries a badge at bottom-right, so a `+` there is two
  ...
  return (
    ...
    >
      {/* **The hole is cut, not painted**, which is the rule `ReferenceIcon` states
          ...
          Resource. `ReferenceIcon` cuts it with an SVG mask because it owns its own
          `svg`; this one is composed over the shipped `ResourceKindIcon`, so it
      ...
      >
        <ResourceKindIcon kind={kind} size={size} />
      </span>
      <svg
...
/**
 * **E — the split.** `+` completes a Markdown Resource; the chevron beside it
 ...
 * *"Create a Markdown Resource"* since before this menu existed. What it genuinely
 * costs is a second target in the cluster — the Resources trigger's chevron, this
  ...
  return (
    <span className="cdp-set-verb inline-flex items-center">
      <ToolbarButton
        ...
        aria-label={`Create ${resourceKindName('markdown')}`}
        title={`Create ${resourceKindName('markdown')}`}
        onClick={() => record(`${resourceKindName('markdown')} — ${KIND_OUTCOME.markdown}`)}
      >
        <PlusIcon />
      </ToolbarButton>
      <DropdownMenu
      ...
      >
        <DropdownMenuTrigger
          ...
          aria-label="Create another kind of Resource"
          title="Create another kind of Resource"
          render={<ToolbarButton variant="ghost" size="icon" />}
        >
          <ChevronDownIcon />
        </DropdownMenuTrigger>
        <DropdownMenuContent align="center" side="top" sideOffset={6} className="nokey w-72">
          <DropdownMenuGroup>
            <DropdownMenuLabel>Create Resource</DropdownMenuLabel>
            {RESOURCE_KINDS.filter((kind) => kind !== 'markdown').map((kind) => (
              <DropdownMenuItem
                key={kind}
                className="gap-2"
                onClick={() => record(`${resourceKindName(kind)} — ${KIND_OUTCOME[kind]}`)}
 ...
 * spent.
 *
 * Create a Markdown Resource once in every strip and read the counts: A costs two,
 * B, C, D and E cost one. Then create a Reference Resource in every strip: A and E cost two,
...
export const Permutations: Story = () => {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background text-foreground">
      <PrototypeBanner>
        Five ways the Resources cluster can offer Create. Every press is counted, disclosures
        included.
      </PrototypeBanner>
      <div className="grid flex-1 gap-4 p-6">
        ...
        <Option
          ...
          cost="the badge lands where the Reference Resource already has one, so Create Reference and Create Markdown Resource draw the same mark. See PlusBadges."
        >
          <DockStrip>
        ...
        <Option
          ...
          cost="a second chevron beside the Resources trigger's own, and a default `CreateMenu`'s own doc argued against."
        >
          <DockStrip>
 ...
 * the chevrons are at one cluster finds them at the next. Resources is already the
 ...
 * what each one does to the column a Map and a Graph also live in. Measured
 ...
 * Left as three siblings they auto-place onto three rows and the Resources block
 * stands at 102px beside a 44px Map and a 44px Graph, which is the Dock
 * losing the alignment its grid is spent on. Wrapped in a nested `ToolbarGroup`
 * the column has one child to place, `create-resource-peer-row.css` gives it
 ...
 * story: ArrowRight on the Resources trigger does nothing, and ArrowDown three
 * times walks Markdown, Space, Reference Resource in the order they are drawn. So the
...
function IdentityRuler({ vertical = true }: { readonly vertical?: boolean }) {
  /* The rule the production Dock already keeps: a `Divider` sits between each
     ...
     clusters bare, which made the Resources row look joined to the Graph above it
  ...
  const rows = [
    { label: 'Meta Space', icon: <ResourceKindIcon kind="space" />, verb: null },
    { label: 'Overview', icon: <MapIcon />, verb: null },
    {
      label: 'Release path',
...
/**
 * B on one line, which is what "Resources" being a fixed word buys.
 *
 ...
 * Space, Map and Graph each name an entity the author renamed, and the
 * length is theirs. Resources names a set, has no name to edit (which is why the
 * word lives inside its trigger rather than beside it), and is the literal
 * string "Resources" in every Space that will ever exist. The 1fr track is 140px
 ...
 * past the edge. The cluster returns to 44px — the same height as Map and
 ...
 * row. Packed Resources puts its own at x=101. But the grid arrangement does not
 * put it at 146 either — it lands at **x=153**, because the Resources trigger spans
 ...
 * 7px seam exactly at Resources, and "the chevrons line up for all four rows" was
 ...
 * from the Resource glyph to the chevron is **122px**, against 53px packed — the
...
export const SingleLine: Story = () => {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background text-foreground">
      <PrototypeBanner>
        B packed onto one row, because "Resources" is a fixed word and needs no slack track.
      </PrototypeBanner>
      <div className="grid flex-1 content-start gap-6 p-6">
        ...
        <section className="grid gap-3 border-t pt-4">
          ...
          <p className="max-w-[46rem] text-xs text-muted-foreground">
            Read down the chevrons. The three identities sit at x=146 in both. Resources sits at
            x=153 on the left and x=101 on the right — so the left is already 7px out of the column,
            and the right trades that column for the peers&rsquo; own 29px pitch. Note the run from
            the Resource glyph to the chevron on the left: 122px against 53px packed, most of it a
            track held open for a word that never changes. Both strips carry the Dock&rsquo;s own
            divider above the Resources row. The three rows above Resources are a ruler, not a
            claim.
          </p>
 ...
 * Markdown Resource badged there is a page with a plus on it. Worse, it is also
 * where a Reference Resource keeps its own arrow, so the `+` does not crowd that badge, it
 * **replaces** it: compare the first and third groups in that row and they are
 * the same mark. Create Markdown Resource and Create Reference would be two commands
 * drawing one glyph.
 *
 * **Top-right survives, at a price the Reference Resource pays alone.** The fold and the
 * Reference Resource arrow both live, so the three kinds stay apart — but the Reference Resource is then
      ...
      {(['bottom-right', 'top-right'] as const).map((corner) => (
        <section key={corner} className="grid gap-3 border-t pt-4">
          <h3 className="text-sm font-semibold">
            Badge at {corner}
            {corner === 'bottom-right'
              ? ' — replaces the Reference Resource badge, and eats the Markdown fold'
              : ' — both silhouettes survive; the Reference Resource carries two badges'}
          </h3>
