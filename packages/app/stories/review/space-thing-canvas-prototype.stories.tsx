 ...
 * change Space Thing authoring. Keep the variants independent enough that the
...
import './space-thing-canvas-prototype.css';

export default { title: 'Review/Space Thing Canvas Architecture' };

type PrototypeVariant = 'nested' | 'compound';
type CanvasScope = 'parent' | 'child';
type DiagramChoice = 'Architecture diagram' | 'Flow view';
type GraphChoice = 'Main thread' | 'Decision fork';
type PresentationThing =
  | 'parent-intro'
  | 'child-entry'
  | 'nested-intro'
  | 'nested-exit'
...
interface SpaceThingState {
  readonly open: boolean;
  readonly activeView: DiagramChoice;
  readonly activeGraph: GraphChoice;
  readonly entered: boolean;
  readonly setOpen: (open: boolean) => void;
  readonly setActiveView: (view: DiagramChoice) => void;
  readonly setActiveGraph: (graph: GraphChoice) => void;
  readonly setEntered: (entered: boolean) => void;
  readonly renameThing: (scope: CanvasScope, id: string, title: string) => void;
...
const INITIAL_CHILD_NODES: PrototypeNode[] = [
  {
    ...
    data: {
      scope: 'child',
      title: 'Child entry',
      body: 'A Thing in the selected Graph; the Space Thing itself is only the bridge.',
      accent: CHILD_GRAPH,
    },
  },
  {
    ...
    data: {
      scope: 'child',
      title: 'Child exit',
      body: 'This Thing can return through an explicit Edge owned by the containing Graph.',
      accent: CHILD_GRAPH,
    },
  },
  {
...
const SpaceThingContext = createContext<SpaceThingState | null>(null);
...
function useSpaceThing() {
  return useRequiredContext(SpaceThingContext, 'SpaceThingContext');
}
...
function PrototypeHandle({
  ...
  return (
    <Handle
      type={endpoint.role}
      position={position}
      className="space-thing-prototype__handle"
...
function MarkdownThingNode({ id, data }: NodeProps<MarkdownNode>) {
  const { renameThing } = useSpaceThing();
  ...
  return (
    <>
      ...
      <Card
        size="sm"
        className="space-thing-prototype__markdown"
        style={accentStyle}
        data-active="false"
      >
        <CardHeader>
          {editing ? (
            <div className="space-thing-prototype__editor nodrag nopan nowheel">
              <Input
                ...
                onKeyDown={(event) => {
                  ...
                  if (event.key === 'Enter' && draft.trim() !== '') {
                    renameThing(data.scope, id, draft.trim());
                    setEditing(false);
                  }
                }}
              />
              <div className="space-thing-prototype__editor-actions">
                <Button
                  size="compact"
                  variant="ghost"
                  onClick={() => {
            ...
            <div className="space-thing-prototype__markdown-title">
        ...
        {!editing && (
          <CardContent>
            <p className="space-thing-prototype__markdown-copy">{data.body}</p>
          </CardContent>
        )}
      </Card>
      <PrototypeHandle
...
function SpaceThingHeader({ independent = false }: { readonly independent?: boolean }) {
  const state = useSpaceThing();
  return (
    <div className="space-thing-prototype__space-header nodrag nopan nowheel">
      <div className="space-thing-prototype__space-title">
        <strong>Research Space</strong>
        <span>{independent ? 'Independent root' : 'Space Thing · linked Space'}</span>
      </div>
      <Select
        value={state.activeView}
        onValueChange={(value) => {
          if (value === 'Architecture diagram' || value === 'Flow view') state.setActiveView(value);
        }}
      >
        <SelectTrigger className="space-thing-prototype__selector" aria-label="Diagram">
          <SelectValue />
        </SelectTrigger>
        <SelectContent alignItemWithTrigger={false}>
          <SelectItem value="Architecture diagram">Architecture diagram</SelectItem>
      ...
      >
        <SelectTrigger className="space-thing-prototype__selector" aria-label="Graph">
          <SelectValue />
        </SelectTrigger>
        <SelectContent alignItemWithTrigger={false}>
...
function ChildCanvas({ compact = false }: { readonly compact?: boolean }) {
  const { childCanvas } = useSpaceThing();
  return (
    <ReactFlowProvider>
      <ReactFlow
        className="space-thing-prototype__child-flow"
...
function NestedSpaceThingNode(): ReactNode {
  const state = useSpaceThing();
  if (!state.open) {
    return (
      <div className="space-thing-prototype__space-node">
        <div className="space-thing-prototype__space-thing" data-open="false">
          <SpaceThingHeader />
          <span className="space-thing-prototype__marker space-thing-prototype__marker--entry">
            →
          </span>
          <span className="space-thing-prototype__marker space-thing-prototype__marker--exit">
            →
          </span>
        </div>
      </div>
  ...
  return (
    <div className="space-thing-prototype__space-node">
      ...
      <div className="space-thing-prototype__space-thing" data-open="true">
        <SpaceThingHeader />
        <div className="space-thing-prototype__space-body nowheel nodrag">
          <ChildCanvas compact />
        </div>
      </div>
    </div>
...
function CompoundSpaceThingNode(): ReactNode {
  const state = useSpaceThing();
  return (
    <div className="space-thing-prototype__space-node">
      {state.open && <NodeResizer minWidth={560} minHeight={390} color={PARENT_GRAPH} />}
      <div className="space-thing-prototype__space-thing" data-open={String(state.open)}>
        <SpaceThingHeader />
        {state.open ? (
          <div className="space-thing-prototype__compound-ground" />
        ) : (
          <>
            <span className="space-thing-prototype__marker space-thing-prototype__marker--entry">
              →
...
function SpaceThingNode({ data }: NodeProps<SpaceNode>) {
  return data.kind === 'nested' ? <NestedSpaceThingNode /> : <CompoundSpaceThingNode />;
}

const NODE_TYPES = { markdown: MarkdownThingNode, space: SpaceThingNode };
...
function NestedBridgeLayer({
  ...
  return (
    <svg className="space-thing-prototype__bridge-layer" aria-hidden="true">
      {drawn.map((edge) => (
        <path
          key={edge.id}
          ...
          className={
            edge.draft
              ? 'space-thing-prototype__bridge-path space-thing-prototype__bridge-path--draft'
              : 'space-thing-prototype__bridge-path'
          }
...
function Presentation({
  ...
  readonly history: readonly PresentationThing[];
  readonly setHistory: (history: readonly PresentationThing[]) => void;
}) {
  const current = history.at(-1);
  if (current === undefined) return null;
  const copy = PRESENTATION_COPY[current];
  const advance = (next: PresentationThing) => setHistory([...history, next]);
  const terminal = current === 'child-stop' || current === 'parent-outro';

  return (
    <section className="space-thing-prototype__presentation" aria-label="Presentation traversal">
      <div className="space-thing-prototype__presentation-path">{copy.context}</div>
      <h2>{copy.title}</h2>
      <p>{copy.body}</p>
      <div className="space-thing-prototype__presentation-actions">
        <Button
          variant="ghost"
...
function VariantSwitcher({
  ...
  return (
    <nav className="space-thing-prototype__switcher" aria-label="Prototype variants">
      <Button
        size="icon"
        variant="ghost"
      ...
      <div className="space-thing-prototype__switcher-copy">
...
function IndependentSpaceRoot() {
  ...
  const [activeView, setActiveView] = useState<DiagramChoice>('Architecture diagram');
  ...
  const context = useMemo<SpaceThingState>(
    () => ({
      open: true,
      activeView,
      activeGraph,
  ...
  return (
    <BridgeContext.Provider value={bridge}>
      <SpaceThingContext.Provider value={context}>
        <main className="space-thing-prototype space-thing-prototype__standalone">
          <header className="space-thing-prototype__root-header">
            <div>
              <strong>Research Space</strong>
          ...
          <div className="space-thing-prototype__standalone-canvas">
            <ChildCanvas />
          </div>
        </main>
      </SpaceThingContext.Provider>
...
function Prototype() {
  ...
  const [activeView, setActiveView] = useState<DiagramChoice>('Architecture diagram');
  const [activeGraph, setActiveGraph] = useState<GraphChoice>('Decision fork');
  const [presentationHistory, setPresentationHistory] = useState<readonly PresentationThing[]>([]);
  const [status, setStatus] = useState(
    initialVariant === 'nested'
      ? 'Drag Things and author local Edges normally. Hold Shift while dragging a handle across the Space boundary.'
  ...
  const setOpen = useCallback(
    (next: boolean) => {
      ...
      setStatus(
        next
          ? 'Space Thing opened on its remembered Diagram and Graph.'
  ...
  const renameThing = useCallback(
    (scope: CanvasScope, id: string, title: string) => {
      ...
      setStatus(`Renamed ${scope} Thing to “${title}”.`);
    },
  ...
  const context = useMemo<SpaceThingState>(
    () => ({
      ...
      setActiveView: (view) => {
        setActiveView(view);
        setStatus(
          `This Space Thing now shows ${view}; the Space's independent selection is unchanged.`,
        );
      },
      setActiveGraph: (graph) => {
        setActiveGraph(graph);
        setStatus(
          `This Space Thing now presents ${graph}; the Space's independent selection is unchanged.`,
        );
      },
      setEntered,
      renameThing,
    ...
    [activeGraph, activeView, childCanvas, entered, open, renameThing, setOpen, standaloneHref],
  );
  ...
  useEffect(() => {
    ...
    const finish = (event: PointerEvent) => {
      ...
      if (target !== null && target.role === 'target' && target.scope !== draft.source.scope) {
        ...
        setStatus('Authored a cross-Space Edge through this Space Thing.');
      } else {
        setStatus(
  ...
  const visibleCompoundNodes = useMemo(
    () =>
      compoundNodes.map((node) =>
        node.parentId === 'space-thing' ? { ...node, hidden: !open } : node,
      ),
    [compoundNodes, open],
  );
  ...
  const architectureNote =
    variant === 'nested'
      ...
      : {
          ...
          body: 'Child Things are subflow nodes, so entry, exit, authoring, and reconnection are native. The embedded Space shares the parent camera.',
        };

  return (
    <BridgeContext.Provider value={bridgeControls}>
      <SpaceThingContext.Provider value={context}>
        <main className="space-thing-prototype">
          <header className="space-thing-prototype__masthead">
            <span className="space-thing-prototype__eyebrow">Throwaway UX prototype</span>
            <h1>Space Thing canvas architecture</h1>
            <p>Same authored scenario, two containment models. Use ←/→ to compare.</p>
          </header>
          <aside className="space-thing-prototype__architecture-note">
            <strong>{architectureNote.title}</strong>
            <p>{architectureNote.body}</p>
          </aside>
          <div className="space-thing-prototype__stage" ref={stageRef}>
            {variant === 'nested' ? (
              <>
                <ReactFlow
                  key="nested-flow"
                  className="space-thing-prototype__flow"
                  nodes={parentNodes}
                  edges={parentEdges}
                  nodeTypes={NODE_TYPES}
          ...
          <div className="space-thing-prototype__status" role="status">
            {status}
            <Button
              className="ml-2"
              size="compact"
          ...
          {entered && (
            <section className="space-thing-prototype__entered" aria-label="Entered Research Space">
              <header className="space-thing-prototype__entered-header">
                <div className="space-thing-prototype__entered-title">
                  <strong>Research Space</strong>
                  <span>
                    Entered from Space Thing · {activeView} · {activeGraph}
                  </span>
                </div>
                <Button onClick={() => setEntered(false)}>Back to containing Space</Button>
              </header>
              <div className="space-thing-prototype__entered-canvas">
                <ChildCanvas />
              </div>
            </section>
          )}
