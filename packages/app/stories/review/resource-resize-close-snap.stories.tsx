import { useState } from 'react';
import type { Story } from '@ladle/react';
import {
  RESOURCE_CLOSE_SNAP_DISTANCE,
  RESOURCE_SIZE,
  resourceSizeVars,
  snapResourceSizeToClose,
} from '#src/resource';
import { CatalogueSection, Specimen } from '../support/Catalogue';
import { CanvasResourceNodeSpecimen } from '../support/ReactFlowCanvas';
import '../support/inventory.css';

export default { title: 'Review/Resource Resize Close Snap' };

interface Proposal {
  readonly width: number;
  readonly height: number;
}

function SnapSpecimen({
  label,
  initialProposal,
  zoom,
}: {
  readonly label: string;
  readonly initialProposal: Proposal;
  readonly zoom: number;
}) {
  const [proposal, setProposal] = useState(initialProposal);
  const snapped = snapResourceSizeToClose(proposal);
  return (
    <Specimen label={`${label} · raw ${proposal.width}×${proposal.height}`}>
      <CanvasResourceNodeSpecimen
        expanded
        selected
        nodeSize={snapped}
        onResize={setProposal}
        stageClassName="inv-resource-node-stage--large"
        zoom={zoom}
      />
    </Specimen>
  );
}

const proposals = [
  {
    label: 'outside · no snap',
    size: {
      width: RESOURCE_SIZE.width + RESOURCE_CLOSE_SNAP_DISTANCE + 1,
      height: RESOURCE_SIZE.height + RESOURCE_CLOSE_SNAP_DISTANCE + 1,
    },
  },
  {
    label: 'entering · exact boundary',
    size: {
      width: RESOURCE_SIZE.width + RESOURCE_CLOSE_SNAP_DISTANCE,
      height: RESOURCE_SIZE.height + RESOURCE_CLOSE_SNAP_DISTANCE,
    },
  },
  {
    label: 'inside · snapped',
    size: {
      width: RESOURCE_SIZE.width + RESOURCE_CLOSE_SNAP_DISTANCE / 2,
      height: RESOURCE_SIZE.height + RESOURCE_CLOSE_SNAP_DISTANCE / 2,
    },
  },
] as const;

/**
 * Review-only tuning surface for the application-owned magnetic distance.
 * Each row feeds the same three canvas-coordinate proposals through the same
 * production snap function at a different viewport zoom. The boundary and
 * inside proposals therefore draw the exact Closed rect while the one-unit
 * outside proposal remains Open-sized.
 */
export const MagneticRange: Story = () => (
  <div className="inv inv-sheet" style={resourceSizeVars}>
    {[0.5, 1, 2].map((zoom) => (
      <CatalogueSection
        key={zoom}
        title={`${zoom}× viewport zoom`}
        note={`${RESOURCE_CLOSE_SNAP_DISTANCE} canvas units; drag any real production control across the boundary to tune it.`}
      >
        <div className="inv-row">
          {proposals.map(({ label, size }) => (
            <SnapSpecimen key={label} label={label} initialProposal={size} zoom={zoom} />
          ))}
        </div>
      </CatalogueSection>
    ))}
  </div>
);
MagneticRange.meta = { iframed: true };
