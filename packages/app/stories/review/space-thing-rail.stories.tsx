 ...
 *   - A Space Thing has no Edit control. Authoring the embedded Space is done by
 *     working in it, so Edit is implicit in the layout rather than a command on
 *     the band.
 *   - A Space Thing's rail carries Select Diagram, Select Graph and Enter,
 ...
 * as `ThingRailActions` (ADR 0073). The roving container below is hand-rolled
 * *for this story only* and is not what shipped — read `ThingRailActions` for
...
import { COLLAPSED_THING_SIZE, DEFAULT_OPEN_SIZE } from '@project/core';
import {
  AbandonEditIcon,
  Button,
  CanvasThing,
...
import './space-thing-rail.css';

export default { title: 'Review/Space Thing Rail' };
...
/** The two Thing kinds this surface compares. `space` is not a domain kind yet. */
type PrototypeKind = 'markdown' | 'space';
type ThingState = 'rest' | 'selected';
/** Whether a rail's action cluster is a plain group or a real toolbar. */
type RailSemantics = 'group' | 'toolbar';

/**
 * The custom properties the prototype publishes to the shipped Thing stylesheet.
 *
 ...
 */
interface ThingSizeStyle extends CSSProperties {
  readonly '--thing-width': string;
  readonly '--thing-height': string;
}

interface PrototypeThingStyle extends ThingSizeStyle {
  readonly '--canvas-thing-graph': string;
}

const sizeVars = (open: boolean): ThingSizeStyle => {
  const size = open ? DEFAULT_OPEN_SIZE : COLLAPSED_THING_SIZE;
  return { '--thing-width': `${size.width}px`, '--thing-height': `${size.height}px` };
};
 ...
 * because entering the Space and expanding the Thing in place are two different
 ...
 * chosen (`review/thing-icons`); they are copied from production and have to
...
function RailActions({
  ...
  if (semantics === 'group') {
    return (
      <div className="canvas-thing__actions" data-rail-semantics="group">
        {children}
      </div>
    );
  }
...
function RailAction({
  ...
  return (
    <Button
      variant="ghost"
      size="icon"
      className="thing__rail-action nodrag nopan"
      aria-label={label}
      {...attributes}
      onClick={() => {
  ...
  return (
    <DropdownMenu modal={false}>
      <DropdownMenuTrigger
        ...
        className="thing__rail-action nodrag nopan nokey"
...
interface PrototypeThingProps {
  ...
  readonly semantics: RailSemantics;
  /** A content edit is running. Only a Markdown Thing has one. */
  readonly editing?: boolean;
  readonly onEditingChange?: (editing: boolean) => void;
}

/**
 * One prototype Thing of either kind, drawn with the shipped Thing and rail CSS.
 *
 * A Markdown Thing's rail carries Edit and Open/Close, and swaps Edit for Save
 * and Cancel while an edit runs. A Space Thing's rail carries Select Diagram,
...
function PrototypeThing({
  ...
  const style: PrototypeThingStyle = {
    ...sizeVars(open),
    '--canvas-thing-graph': GRAPH_COLOR,
  };
  const kindName = kind === 'space' ? 'Space Thing' : 'Markdown Thing';

  return (
    <div
      className="canvas-thing space-rail__thing"
      data-testid="thing"
      data-kind={kind}
      data-state={state}
      data-expanded={open}
      ...
      style={style}
    >
      <div className="thing-rail canvas-thing__rail" style={{ background: GRAPH_COLOR }}>
        <span className="thing-rail__kind" role="img" aria-label={kindName} title={kindName}>
          {kind === 'space' ? <SpaceKindGlyph /> : <MarkdownIcon />}
        </span>
        <RailActions semantics={semantics} label={`Thing ${title}`}>
          {kind === 'space' && (
            <>
              <RailSelect
                label="Diagram"
          ...
          {kind === 'markdown' &&
            (editing ? (
              <>
                ...
                <RailAction
                  label={`Cancel editing Thing ${title}`}
                  semantics={semantics}
                  keyshortcuts="Escape"
                  onClick={() => onEditingChange?.(false)}
                >
          ...
          <RailAction
            label={`${open ? 'Close' : 'Open'} Thing ${title}`}
            semantics={semantics}
            unavailable={editing}
      ...
      {open && (
        <div className="canvas-thing__content space-rail__content" data-presence="present">
          {kind === 'space' ? (
      ...
      <div className="canvas-thing__body">
        <h2 className="canvas-thing__title">{title}</h2>
      </div>
    </div>
  );
}

/** The Open Markdown Thing, drawn by the shipped component, as the reference. */
function MarkdownReference({ state }: { readonly state: ThingState }) {
  const [open, setOpen] = useState(true);
  return (
    <div className="space-rail__thing" style={sizeVars(open)}>
      <CanvasThing
        title="Traversal"
...
export const RailComposition: Story = () => (
  <div className="inv inv-sheet">
    <CatalogueSection
      title="1 · The Space Thing rail"
      note="Things are drawn Selected so their rails are up; a resting Thing on the canvas hides them until hover. Every rail control is the same 22px icon button. A Space Thing carries Select Diagram, Select Graph, Enter and Open/Close — four glyphs, no words on the band. Diagram and Graph open dropdowns; their current value is the control's accessible name and the marked item in the menu. Enter draws Lucide log-in — an arrow into a container — deliberately unlike Open's maximize-2, because entering the Space and expanding the Thing in place are different destinations."
    >
    ...
    <CatalogueSection
      ...
      note="The one remaining choice, and it draws no pixels — the two variants are identical to look at and differ only in what the keyboard does, which is why each specimen prints what a Tab actually reaches inside it. The Space pair: a plain group costs four tab stops per Thing, a toolbar costs one and the arrows move between the four. The Markdown pair: press Edit on each and watch the order. The group's becomes Save → Cancel, so Close has left the keyboard entirely although it is still drawn — ADR 0064's promise that it keeps its slot holds for the eye only. The meter counts tab stops, so it cannot show the other half: in the toolbar variant that unavailable Close is still there under the arrow keys."
    >
      <div className="inv-row space-rail__row">
        <SpaceSpecimen
