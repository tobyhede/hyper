...
test(
  ...
  { tag: '@parity:canvas-thing-shows-rest-selected-and-dragging-states' },
  async ({ page }) => {
    await page.goto('/?story=components--thing--states&mode=preview');

    const rest = specimen(page, 'thing · rest').getByRole('article');
    const selected = specimen(page, 'thing · selected').getByRole('article');
    const dragging = specimen(page, 'thing · dragging').getByRole('article');
...
const FRONTS = [
  ...
  { label: 'creation ghost', kind: 'markdown', glyph: 'Markdown Thing', border: 'solid' },
] as const;
...
/**
 * The whole of what a Thing front draws, and the whole of what it does not.
 *
 * This is the one place that states it. Every other Thing story is a slice —
 ...
 * over the Thing's own box: the kind glyph, the border, one element per Title
 ...
 * Thing that is not one of the Title Lines the author typed.
 */
test(
  'every Thing front draws its kind, its border and its Title Lines, and nothing beneath them',
  { tag: '@parity:canvas-thing-front-draws-only-its-title-lines' },
  async ({ page }) => {
    await page.goto('/?story=components--thing--front&mode=preview');
    ...
    for (const front of FRONTS) {
      for (const [suffix, expected] of [
        ...
        const thing = specimen(page, `${front.label} · ${suffix}`).getByRole('article');
        await expect(thing).toHaveAttribute('data-kind', front.kind);
        await expect(thing).toHaveAttribute('data-state', 'rest');
        await expect(thing).toHaveAttribute('data-expanded', 'false');
        await expect(thing.getByRole('img', { name: front.glyph })).toBeVisible();
        await expect(thing).toHaveCSS('border-style', front.border);
        ...
        await expect(thing.locator('.canvas-thing__body > .canvas-thing__title')).toHaveCount(1);
        await expect(thing.locator('.canvas-thing__content')).toHaveCount(0);
        const text = (await thing.innerText())
          .split('\n')
          .map((line) => line.trim())
          .filter((line) => line !== '');
    ...
    const rungs = await sizesOf(ladder.locator('.canvas-thing__title-line'));
 ...
 * **box** chose does not.
 */
// Untagged: the parity claim `canvas-thing-front-draws-only-its-title-lines`
...
test('a wrapped single-line Title stays one rung while an authored three-line Title draws three', async ({
  ...
  const wrapped = specimen(page, 'one Title Line, wrapped')
    .getByRole('article')
    .locator('.canvas-thing__title-line');
...
test(
  ...
  async ({ page }) => {
    ...
    // the thing to fit a title that overruns it.
    ...
    const lines = longTitle.locator('.canvas-thing__title-line');
...
test(
  "hovering the real React Flow node reveals CanvasThing's rail actions and the adapter's Edge handles together",
  { tag: '@parity:canvas-thing-hover-reveals-actions-and-handles-together' },
  async ({ page }) => {
...
test(
  ...
  async ({ page }) => {
    ...
    const open = page.getByRole('button', { name: 'Open Thing Strategies' });
    ...
    const titleTreatment = async () => {
      ...
      const thingBox = await markdown.boundingBox();
      if (titleBox === null || thingBox === null)
        throw new Error('Thing title geometry unavailable');
      return {
        left: titleBox.x - thingBox.x,
        bottom: thingBox.y + thingBox.height - (titleBox.y + titleBox.height),
    ...
    await expect(page.getByRole('button', { name: 'Close Thing Strategies' })).toBeVisible();
    await expect.poll(() => titleTreatment()).toEqual(closedTitleTreatment);
  },
);

test('reduced motion removes Thing content and rail-action transitions', async ({ page }) => {
  await page.emulateMedia({ reducedMotion: 'reduce' });
  await page.goto('/?story=components--thing--open-and-close&mode=preview');

  const thing = page.getByRole('region', { name: 'Long Markdown Thing' }).getByRole('article');
  const content = thing.locator('.canvas-thing__content');
  const action = thing.getByRole('button', { name: 'Close Thing Long Markdown' });
...
test(
  "the Thing's Title control begins editing by pointer or keyboard, stays field-local on a refusal, and completes or cancels from the keyboard",
  { tag: '@parity:canvas-thing-owns-title-editing-and-refusal' },
  async ({ page }) => {
    await page.goto('/?story=components--thing--editing--title&mode=preview');

    const closedSpecimen = page.getByRole('region', { name: 'Closed Thing title editing' });
    const group = closedSpecimen.getByTestId('thing-group');
    ...
    // regression ADR 0083's sentence about the Thing front's own heading exists
    ...
    const ladderGroup = page
      .getByRole('region', { name: 'Laddered Thing title editing' })
      .getByTestId('thing-group');
    ...
    // `canvas-thing.css` draws it on `.canvas-thing__title:has(…__title-control:hover)`
    // so the tint and rule span the whole Title rather than the text's own box.
    const titleBand = group.locator('.canvas-thing__title');
    await expect
    ...
    const input = page.getByRole('textbox', { name: 'Thing title' });
    ...
    // A valid Enter completes the draft and hands focus back to the Thing.
    ...
    const reopened = page.getByRole('textbox', { name: 'Thing title' });
    ...
    // The dedicated Title story also proves that opening a Thing does not
    // introduce a second title editor or hide the Thing body while it runs.
    const openSpecimen = page.getByRole('region', { name: 'Open Thing title editing' });
    const openGroup = openSpecimen.getByTestId('thing-group');
    await expect(openGroup.getByRole('heading', { name: 'Open Thing body' })).toBeVisible();
    await openGroup.getByRole('button', { name: 'Edit Title Draft entry' }).click();
    await expect(openGroup.getByRole('textbox', { name: 'Thing title' })).toBeFocused();
    await expect(openGroup.getByRole('heading', { name: 'Open Thing body' })).toBeVisible();
  },
);
 ...
 * Thing whose height is authored. Uncapped, the fifth line took the Thing's own
 * rail with it: the body is bottom-pinned, so it grows *upward*, and the rail
 * above it was pushed out of the top of the Thing and clipped away by the Thing's
 ...
 * `packages/ui/test/canvas-thing-title-ladder.test.ts`.
 */
test('a Title being written on more lines than fit does not push the rail out of its Thing', async ({
  page,
}) => {
  await page.goto('/?story=components--thing--editing--title&mode=preview');
  ...
  const group = page
    .getByRole('region', { name: 'Laddered Thing title editing' })
    .getByTestId('thing-group');
  const thing = group.locator('.canvas-thing');
  const rail = group.locator('.thing-rail');
  await group.getByRole('button', { name: 'Edit Title Auth' }).click();

  const field = page.getByRole('textbox', { name: 'Thing title' });
  await expect(field).toBeFocused();

  /** The field's drawn height, and whether the rail is still inside the Thing. */
  const geometry = async () => {
    const thingBox = await thing.boundingBox();
    const railBox = await rail.boundingBox();
    const fieldBox = await field.boundingBox();
    if (thingBox === null || railBox === null || fieldBox === null) {
      throw new Error('Thing geometry unavailable');
    }
    return {
      ...
      railAboveThing: railBox.y < thingBox.y,
    };
  };
  ...
  expect(atCeiling.railAboveThing).toBe(false);
  ...
  expect(pastCeiling.railAboveThing).toBe(false);
