...
test(
  ...
  async ({ page }) => {
    ...
    // The story's failure carries a code and no prose: `authoring-refusal.ts`
    // owns what the author reads, keyed by that stable code — here `network`.
...
test(
  ...
  async ({ page }) => {
    ...
    // `forbidden`'s application-owned sentence, for the reason above.
