 ...
 * first. Advancing there as well moved two Resources for one press; preventing the
 * default instead stopped the button firing at all. Landing on `Wrap up` is the
 * first defect and landing nowhere is the second, so the assertion is the Resource
 * one move on.
 */
test(
...
test(
  ...
  async ({ page }) => {
    ...
    // ends where it arrives, so the Resource reached is a sink.
    await expect(page.getByTestId('presenting-end')).toBeVisible();
...
/**
 * The chrome reads its own width rather than the viewport's, so a narrow
 * *region* is the same question a narrow window asks.
 */
test(
  ...
  async ({ page }) => {
    ...
    // over the presented Resource either: one bounded row that scrolls.
    const moves = page.getByTestId('presenting-moves');
    await expect(moves.getByRole('button')).toHaveCount(4);

    // A Resource's title has no length limit, so the longest choice is ellipsized
