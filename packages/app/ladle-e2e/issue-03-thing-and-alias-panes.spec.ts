...
test(
  ...
  async ({ page }) => {
    ...
    // The Markdown Thing's own id, so the story reports the Target it was handed
...
/**
 * The Thing-choice popup's paper theme, pinned for the same reason as the editor's
 * above: it now survives only on the side-effect `import './thing-search-combobox.css'`
 * in `ThingSearchCombobox`, and every behavioural assertion in both suites passes
 ...
 */
test('the Thing-choice popup draws its paper theme from the component that owns it', async ({
  page,
}) => {
  await page.goto('/?story=components--alias-panes--new-alias-pane&mode=preview');
  ...
  const popup = page.locator('[data-thing-search-combobox]');
