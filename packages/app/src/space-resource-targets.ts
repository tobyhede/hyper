...
export const useSpaceResourceTargets = (
   ...
   * render and nothing is read for it. Adjusted during render rather than
   * installed after an empty batch settles, so a Space without Space Resources
   * renders no second time on a microtask nobody awaits, and the last Space
   * Resource's target goes on the render that removes it.
   */
  const referencesNothing = !resources.some((resource) => resource.kind === 'space');
  ...
  useEffect(() => {
    ...
    // Nothing referenced is nothing to read: the render below has already
    // dropped any targets, and a read still in flight is invalidated above.
