 ...
 * edge ids. It owns `resource | edge | canvas` and is the only place a *continuation*
...
export function CanvasContinuation({
   ...
   * spare.** A resource target waits on exactly the same schedule an Edge does, and
   ...
   * create-and-connect takes — Authoring mints a Resource, the projection carrying
   ...
   * resource target is resolved by `data-id` and consults no list.
   */
  ...
  useEffect(() => {
    ...
    if (target.kind !== 'resource' && target.kind !== 'edge' && target.kind !== 'canvas') return;
    ...
    // drop. `rename` is the created Resource's own inline editor, which `App` opens
    // by passing the Resource down and which focuses itself; putting a `.focus()`
