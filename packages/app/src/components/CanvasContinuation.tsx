 ...
 * edge ids. It owns `thing | edge | canvas` and is the only place a *continuation*
 ...
 * there being Things on the canvas, so an adapter inside it would never spend a
...
export function CanvasContinuation({
   ...
   * spare.** A thing target waits on exactly the same schedule an Edge does, and
   ...
   * create-and-connect takes — Authoring mints a Thing, the projection carrying
   ...
   * thing target is resolved by `data-id` and consults no list.
   */
  ...
  const elementOf = (target: ContinuationTarget): HTMLElement | null => {
    ...
    if (target.kind === 'thing') {
      return document.querySelector<HTMLElement>(
        `.react-flow__node[data-id="${CSS.escape(target.thingId)}"]`,
      );
    }
  ...
  useEffect(() => {
    ...
    if (target.kind !== 'thing' && target.kind !== 'edge' && target.kind !== 'canvas') return;
    ...
    const drawn =
      ...
      (pending.then !== 'reveal' ||
        target.kind !== 'thing' ||
        flow.getNode(target.thingId) !== undefined);
    if (!drawn && staysOwed(pending)) return;
    if (pending.select) {
      if (target.kind === 'thing') onSelectThing(target.thingId);
      if (target.kind === 'edge') onSelectEdge(target);
    }
    continuation.take();
    // `nothing` is a gesture that owed a subject and no caret — a completed Edge
    // drop. `rename` is the created Thing's own inline editor, which `App` opens
    // by passing the Thing down and which focuses itself; putting a `.focus()`
    // on the node here would take the caret straight back off it.
    if (pending.then === 'nothing' || pending.then === 'rename') return;
    if (pending.then === 'reveal' && target.kind === 'thing' && drawn) {
      // The camera, and the only member that touches it: a Thing addressed by
      // URL is somewhere the reader has never been.
      void flow
        .fitView({ nodes: [{ id: target.thingId }], padding: 0.5, duration: 0 })
        .then(() => element.focus());
      return;
    }
