...
import type { SpaceThingTargetListing } from '../thing-creation';
 ...
 * choices produce the same Thing and differ only in whether the Space it names
 * already exists (ADR 0068). A radio pair above the list would have said there
 * were two kinds of Space Thing.
 */
const NEW_SPACE = 'new';
...
export interface NewSpaceThingProps {
  /**
   * Every Space this Thing may reference — the containing Space is already
   * withheld — and, until the read answers, the fact that there is not yet a
   * list to choose from.
   */
  readonly targets: SpaceThingTargetListing;
  /**
   * Why the last attempt produced no Thing, already placed on the fields this
   * pane owns, or `null`.
   *
  ...
  /**
   * Create the Space Thing, against a Space that exists or a Space this creates.
   *
...
/**
 * Adding a Space Thing: the Thing editor, opened on a Thing that does not exist yet.
 *
 * **One typed title seeds two things** — this Thing and the Space it references
 ...
 * which they are not. The new Space's first Markdown Thing is *not* one of them:
 * it takes the neutral `Thing 1` the one Space initializer mints, because a
 * Thing titled after the Space it lives in only reads as deliberate until the
 * first rename makes the pair disagree.
 *
 * Unlike Alias creation, choosing the target is *not* the completion. An Alias
 * without a Target is not a valid Thing, so the choice is the last thing missing;
 * a Space Thing always has a valid target available — a new Space — so what is
 ...
 * answer on its own, so without that the author can title the Thing and create a
 ...
 * become a Space Thing — a Thing keeps the kind it was created with — so the pane
 * says which kind it is creating rather than offering a control that changes it.
 * Neither is the *target*: a Space Thing is never retargeted (ADR 0068), so this
...
export function NewSpaceThing({
  ...
  /**
   * The Title as it will be stored, which is also what says whether the Thing
   * has been named.
   *
   * `normalizeTitle` and not `trim()`, because the two disagree and the Thing's
   ...
   * There is no schema between this and the stored Thing: the coordinated
   ...
   * new Space's own single-line title — takes the Thing's answer rather than a
  ...
  return (
    <ThingPane
      ariaLabel="New Space Thing"
      testId="new-space-thing"
      ...
      <div className="thing-pane__editor" onChange={refusal === null ? undefined : onRefusalStale}>
        <FieldGroup className="thing-pane__fields">
          ...
          <Field className="thing-pane__heading">
            <FieldTitle>New Space Thing</FieldTitle>
            <FieldDescription>
              A Space Thing shows another Space, through the Diagram and Graph it selects.
            </FieldDescription>
          </Field>
          <Field className="thing-pane__field" data-invalid={titleError !== null}>
            <FieldLabel htmlFor="new-space-thing-title">Title</FieldLabel>
            <Input
              id="new-space-thing-title"
              data-testid="new-space-thing-title"
              value={title}
              aria-invalid={titleError !== null}
              aria-describedby={titleError === null ? undefined : 'new-space-thing-title-error'}
              {...paneInitialFocus(true)}
              onChange={(event) => setTitle(event.currentTarget.value)}
            />
            <FieldError id="new-space-thing-title-error">{titleError}</FieldError>
          </Field>
          <Field className="thing-pane__field" data-invalid={targetError !== null}>
            <FieldLabel htmlFor="new-space-thing-target">Space</FieldLabel>
            <Select
              value={target}
            ...
            >
              <SelectTrigger
                id="new-space-thing-target"
                data-testid="new-space-thing-target"
                className="w-full"
                aria-invalid={targetError !== null}
                aria-describedby={targetError === null ? undefined : 'new-space-thing-target-error'}
              >
              ...
              {/* `nokey` because the popup is portalled out of this pane and
                  ...
                  (docs/agents/ui.md). */}
              <SelectContent className="nokey" data-testid="new-space-thing-target-options">
            ...
            <FieldError id="new-space-thing-target-error">{targetError}</FieldError>
          </Field>
          {refusal === null && (
            <FieldDescription className="thing-pane__hint">
              A new Space begins with one Markdown Thing. Referencing an existing Space adds a
              second way to reach it, and never a copy.
            </FieldDescription>
          )}
        </FieldGroup>
        {formError !== null && (
        ...
        <div className="thing-pane__actions">
          ...
          <Button
            type="button"
            data-testid="new-space-thing-create"
            onClick={create}
