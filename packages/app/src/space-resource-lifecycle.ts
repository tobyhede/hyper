...
import type { GraphHeadShape, GraphId, Map, UUID } from '@project/core';
...
export interface SpaceResourceTargetMap {
   ...
   * is the only Space that knows them, and through `graphAppearance` because
   * the canvas resolves them there too.
   */
...
export interface SpaceResourceAuthoring extends SpaceResourceLifecycle {
   ...
   * Resources that formed it, which is a better sentence than a silently shorter
   * list (ADR 0068, ADR 0074).
   */
  readonly referenceableSpaces: (containingSpaceId: UUID) => Promise<readonly SpaceSummary[]>;
  /**
   * What one target Space offers a Space Resource to select, or `undefined` where
...
export function createSpaceResourceLifecycle({
  ...
  return {
    ...
    deleteMap: announcing(lifecycle.deleteMap),
    ...
    target: async (spaceId) => {
      ...
      return spaceResourceTarget(loaded.space);
    },
  };
}
