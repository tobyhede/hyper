...
/** What the canvas draws, given the placement and whether Resources are on screen. */
...
/**
 * A pending placement is not by itself a reason to blank the canvas. Once Resources
 ...
 * another Map is the case that does clear them, which is why that is the
 * editor's decision and not this one.
 */
export function canvasContent(
  placement: PlacementRenderingState,
  hasResourcesOnCanvas: boolean,
): CanvasContent {
  if (placement.kind === 'failed') return { kind: 'failure', error: placement.error };
  if (hasResourcesOnCanvas) return { kind: 'resources' };
