...
import {
  ...
  type EmbeddedOpenSpaceThingRequest,
...
const EMPTY_PORTALS: ReadonlySet<ThingId> = new Set();
...
export interface EmbeddedTargetReader {
  ...
  readonly embed: (spaceId: ThingId) => Promise<OpenSpace | undefined>;
}

export interface EmbeddedOpenSpaceThings {
  readonly embeddedRequests: readonly EmbeddedOpenSpaceThingRequest<OpenSpace>[];
  readonly embeddedPublications: ReadonlyMap<string, EmbeddedPublication>;
  readonly embeddedFailures: ReadonlyMap<ThingId, string>;
  readonly resumeEmbedded: (spaceId: ThingId) => Promise<void>;
  ...
  readonly editingPortals: ReadonlySet<ThingId>;
  readonly onPortalEditingChange: (thingId: ThingId, editing: boolean) => void;
  readonly portalDraft: ReadonlyMap<ThingId, SpaceThingFraming>;
  readonly setPortalDraft: Dispatch<SetStateAction<ReadonlyMap<ThingId, SpaceThingFraming>>>;
}

/**
 * Target-read scheduling, embed failures, publication registry, and edit-portal
 * state for nested Open Space Things. Mounting and clip-path stay with the canvas.
 */
export function useEmbeddedOpenSpaceThings(
  ...
  const [editingPortals, setEditingPortals] = useState<ReadonlySet<ThingId>>(EMPTY_PORTALS);
  const [portalDraft, setPortalDraft] = useState<ReadonlyMap<ThingId, SpaceThingFraming>>(
    () => new Map(),
  );
  const onPortalEditingChange = useCallback((thingId: ThingId, editing: boolean) => {
    setEditingPortals((previous) => {
      const has = previous.has(thingId);
   ...
   * from a Diagram nobody is reading any more.
   *
  ...
  // `use-embedded-open-space-things.test.ts` ('drops a failure whose embedding
