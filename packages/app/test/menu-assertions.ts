 ...
 * Map/Graph list item's own checked-radio indicator.
 *
 * `EntityActionItems` (the Resource and Space Resource actions menu) draws an icon
 * in an `aria-hidden` column ahead of a label wrapper — and an icon that is
 * itself a `ResourceKindIcon` (Create Reference's) nests a second `span`, so a bare
 ...
 * description row.
 *
 * `DropdownMenuRadioItem` (the Map/Graph list) needs the same exclusion
 ...
 *
 * `MapMenuActions` and `GraphMenuActions` (the Map and Graph command
