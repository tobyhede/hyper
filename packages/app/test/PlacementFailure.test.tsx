...
describe('a placement failure', () => {
  it('announces itself, carrying what the strategy said', () => {
    render(<PlacementFailure error={new Error('No position for Resource A')} />);

    expect(screen.getByRole('alert')).toHaveTextContent('No position for Resource A');
  });
  ...
  /**
   * A strategy's message says which Resource it could not place; it does not say
  ...
  it('frames the failure rather than handing over the strategy message alone', () => {
    ...
    expect(heading).not.toHaveTextContent('No position for Resource A');
  });
});
