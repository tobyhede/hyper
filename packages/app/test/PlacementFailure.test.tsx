...
describe('a placement failure', () => {
  it('announces itself, carrying what the strategy said', () => {
    render(<PlacementFailure error={new Error('No position for Thing A')} />);

    expect(screen.getByRole('alert')).toHaveTextContent('No position for Thing A');
  });
  ...
  it('frames the failure rather than handing over the strategy message alone', () => {
    ...
    expect(heading).not.toHaveTextContent('No position for Thing A');
  });
});
