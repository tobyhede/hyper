 ...
 * guidance claims are available. Whether a traversal arrives at the right Resource
...
const movesTo = (titles: readonly string[], selectedIndex: number): readonly Move[] =>
  titles.map((title, index) => {
    const resourceId = RESOURCE_IDS[index];
    if (resourceId === undefined) throw new Error('The fixture declares no id for this move.');
    return { resourceId, title, selected: index === selectedIndex };
  });
...
describe('PresentingChrome', () => {
  ...
  it('names the action each move performs rather than the state it is in', () => {
    ...
    // The visible text stays the Resource's title, so what is written on the control
   ...
   * resource: what the presenter can do from the Resource they are on. A changed choice
   ...
   * (`docs/agents/ui.md`, as `CanvasResource`'s Save and Cancel rail actions do). The visible `Kbd` guidance is
   ...
   *
   * Entry is from outside this chrome — the Dock's Present, which hides while
   * presenting, or an address — so nothing in the chrome holds focus when it
   * mounts. `usePresentingKeys` leaves Space to whatever control has focus, so
   * the chrome claims focus as it mounts: Space then presses the move that
   * advances.
   */
   ...
   * is rebuilt from the Resource arrived at — so the chrome owes focus to whatever
   * took their place.
   */
   ...
   * rather than a block that wraps over the Resource being presented. A selection
