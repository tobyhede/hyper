 ...
 * names the parent and nothing above it, and the switcher holds the rest. That
...
describe('what the bar draws above the Space you are in', () => {
  it('names the parent and never the crossing above it', () => {
    ...
    expect(trailControls(step('Tooling'), rows(7))).toBe('parent-and-switcher');
  });
  ...
  it('keeps the switcher at the root, which is otherwise the one place with no way back', () => {
    // No parent step to spend a name on, so the switcher arrives one Space
    // earlier than it does below the root.
    expect(trailControls(null, rows(2))).toBe('switcher');
  });

  it('leaves the switcher off while the parent step already names the whole set', () => {
    expect(trailControls(step('Platform'), rows(2))).toBe('parent');
  });

  it('discloses the Space after the ones the bar is already naming', () => {
    expect(trailControls(step('Platform'), rows(3))).toBe('parent-and-switcher');
  });
});
