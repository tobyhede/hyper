...
describe('canvasContent', () => {
  const placed: LayoutStrategyGraph = {
    resources: [{ id: RESOURCE_ID, width: 260, height: 146, x: 0, y: 0 }],
    edges: [],
  };

  it('waits for the editor to take a ready placement before drawing it', () => {
    // A resolved placement is not yet Resources on the canvas: `syncProjection`
  ...
  it('keeps drawing the Resources on the canvas while a replacement placement is pending', () => {
    // The Resources on the canvas belong to the editor, which owns their positions
    ...
    expect(canvasContent({ kind: 'pending' }, true)).toEqual({ kind: 'resources' });
  });

  it('reports a failed placement even when Resources are on the canvas', () => {
