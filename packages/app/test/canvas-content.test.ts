...
describe('canvasContent', () => {
  ...
  it('keeps drawing the Things on the canvas while a replacement placement is pending', () => {
    // The Things on the canvas belong to the editor, which owns their positions
    ...
    expect(canvasContent({ kind: 'pending' }, true)).toEqual({ kind: 'things' });
  });

  it('reports a failed placement even when Things are on the canvas', () => {
