...
/** The Graph identity that second conversion would carry with it. */
const COMPETING_GRAPH_ID = uuidSchema.parse('00000000-0000-4000-8000-000000000a02');
 ...
 *
 * Each option is spelled `| undefined` so it forwards into `composeApp`
 * untouched — the composition declares its own the same way, which is what
 * retires the conditional-spread block this pair used to need.
 */
interface AuthoringOptions {
  readonly reportObserverError?: ((error: unknown) => void) | undefined;
  readonly newId?: (() => UUID) | undefined;
}

/**
 * The Graph identities a composition's conversions mint.
 *
 ...
 * composition is what takes that one.
 *
...
const openConflictedAgainstStoredSpace = async () => {
      ...
      layouts: [
        {
          ...
          kind: 'positioned',
          positions: {
            [CARD_A]: { x: 900, y: 700, state: 'closed' },
            [CARD_B]: { x: 600, y: 500, state: 'closed' },
          },
          graphs: [MAIN_GRAPH],
        },
      ],
  ...
  replacePlacementForTest(
    authoring,
    Placement.fromEntries([
      [CARD_A, { x: 500, y: 600, state: 'closed' }],
      [CARD_B, { x: 300, y: 40, state: 'closed' }],
    ]),
  );
...
describe('Space Authoring', () => {
        ...
        layouts: [
          ...(positionedSnapshot.document.layouts ?? []),
          {
            ...
            positions: {
              ...
              [CARD_B]: { x: 320, y: 400, state: 'closed' },
            },
        ...
        layouts: [
          {
            ...
            title: 'Layout 1',
            kind: 'positioned',
            positions: { [CARD_A]: { x: 10, y: 20, state: 'closed' } },
  ...
  it('creates the Card, first Graph, Edge and Layout as one Edit with internal identities', () => {
    ...
    replacePlacementForTest(
      authoring,
      Placement.fromEntries([[CARD_A, { x: 120, y: 240, state: 'closed' }]]),
    );
        ...
        layouts: [
          {
            ...
            kind: 'positioned',
            positions: {
              [CARD_A]: { x: 120, y: 240, state: 'closed' },
  ...
  it('queues a reentrant completion behind publication of the fully installed Edit', () => {
    ...
    authoring.subscribe(() => {
      ...
      replacePlacementForTest(
        ...
        Placement.fromEntries([
          [CARD_A, { x: 10, y: 20, state: 'closed' }],
          [CARD_B, { x: 500, y: 400, state: 'closed' }],
        ]),
      );
  ...
  it('reports a failed queued completion instead of charging it to the Edit that drained it', () => {
    ...
    authoring.subscribe(() => {
      ...
      replacePlacementForTest(
        ...
        Placement.fromEntries([
          [CARD_A, { x: 10, y: 20, state: 'closed' }],
          [UNKNOWN_CARD, { x: 700, y: 800, state: 'closed' }],
        ]),
      );
      complete(authoring, { kind: 'settled-card-movement' });
    });
  ...
  it('keeps persistence failure visible, accepts another Edit, and retries the latest Space', async () => {
    ...
    replacePlacementForTest(
      authoring,
      Placement.fromEntries([
        [CARD_A, { x: 100, y: 200, state: 'closed' }],
        [CARD_B, { x: 300, y: 40, state: 'closed' }],
      ]),
    );
    complete(authoring, { kind: 'settled-card-movement' });
  ...
  it('answers the installed placement from one accessor, and keeps identity for an equal one', () => {
    ...
    expect(installed).toEqual(Placement.fromEntries([[CARD_A, { x: 10, y: 20, state: 'closed' }]]));
      ...
      fc.property(
        ...
        ({ baseAX, baseAY, baseBX, baseBY, renderedAX, renderedAY, renderedBX, renderedBY }) => {
          const base = Placement.fromEntries([
            [CARD_A, { x: baseAX, y: baseAY, state: 'closed' }],
            [CARD_B, { x: baseBX, y: baseBY, state: 'closed' }],
          ]);
          const rendered = Placement.fromEntries([
            [CARD_A, { x: renderedAX, y: renderedAY, state: 'closed' }],
            [CARD_B, { x: renderedBX, y: renderedBY, state: 'closed' }],
          ]);
  ...
  it('discards a queued completion written against the Space a replacement replaced', async () => {
    ...
    authoring.subscribe(() => {
      ...
      replacePlacementForTest(
        authoring,
        Placement.fromEntries([
          [CARD_A, { x: 111, y: 222, state: 'closed' }],
          [CARD_B, { x: 300, y: 40, state: 'closed' }],
        ]),
      );
    ...
    replacePlacementForTest(
      authoring,
      Placement.fromEntries([
        [CARD_A, { x: 700, y: 800, state: 'closed' }],
        [CARD_B, { x: 300, y: 40, state: 'closed' }],
      ]),
    );
    expect(complete(authoring, { kind: 'settled-card-movement' })).toEqual({ kind: 'completed' });
