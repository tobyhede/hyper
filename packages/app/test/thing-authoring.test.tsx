...
/**
 * Two Things on one Graph the Diagram owns, so the graph opens on a Positioned
    ...
    diagrams: [
      {
        ...
        positions: {
          ...
          [OTHER_THING_ID]: { x: 300, y: 20, open: false },
        },
        graphs: [{ id: GRAPH_ID, title: 'Graph', edges: [{ from: THING_ID, to: OTHER_THING_ID }] }],
      },
    ],
...
const thingTitleOf = (session: SpaceSession, thingId: string): string | undefined =>
  session.getState().working.things.find((thing) => thing.id === thingId)?.document.title;

const bodyOf = (session: SpaceSession, thingId: string): string | undefined => {
  const document = session
    .getState()
    .working.things.find((thing) => thing.id === thingId)?.document;
  return document?.kind === 'markdown' ? document.body : undefined;
};

/**
 * Persistence is asynchronous and the strategy that places Things is too, so a
...
describe('authoring a Thing title on the graph', () => {
  /**
   * `z.string().min(1)` counts characters, and a space is one — so the schema
   * alone accepts a title that draws as nothing, leaving a Thing that cannot be
   * told apart from its neighbours and an `Edit title of` label naming nobody.
   * Blank is the empty case wearing different bytes.
   */
  it('refuses a blank title and leaves the stored Thing alone', async () => {
    const session = mount();
    ...
    expect(thingTitleOf(session, THING_ID)).toBe('A');
   ...
   * with no nodes at all and the edited Thing unmounts — taking the draft text,
   ...
   */
  it('withdraws chrome authoring while a Thing title editor holds a refused draft', async () => {
    const session = mount();
    ...
    // so the press below would reach the Thing only by whatever jsdom happens to
    ...
    expect(screen.getByRole('textbox', { name: 'Thing title' })).toHaveAttribute(
      'aria-invalid',
      'true',
    );
    expect(screen.getByRole('alert')).toHaveTextContent('A Thing title is required.');
   ...
   * other one. Every entity Edit is withdrawn while a Thing title editor holds a
   ...
   * nothing.
   */
  it('withdraws Delete Diagram while a Thing title editor holds a refused draft', async () => {
   ...
   */
  it('withdraws New Graph while a Thing title editor holds a refused draft', async () => {
    const session = mount();
    await screen.findByTestId('selected-canvas');
  ...
  it('stores a Title normalized the way the schema normalizes it', async () => {
    ...
    expect(thingTitleOf(session, THING_ID)).toBe('Renamed A\nA subtitle');
    await settled(session);
  });

  /**
   * A Title written on more than one line reaches the Thing whole (ADR 0083).
   *
  ...
  it('stores a Title written on more than one line', async () => {
    ...
    expect(thingTitleOf(session, THING_ID)).toBe(
      'Auth\nThe service, not the screen\n\nOwned by platform',
    );
    await settled(session);
  });
 ...
 * after their first edit on the Flow view. `graphStartThing` has no answer for
...
/** Open Thing A in place, then put its Markdown body under the caret. */
async function openEditor(): Promise<void> {
  fireEvent.click(await screen.findByRole('button', { name: 'Open Thing A' }));
  await screen.findByTestId('markdown-thing-body-edit-target');
  fireEvent.click(await screen.findByRole('button', { name: 'Edit Thing A' }));
...
describe('authoring an opened Thing', () => {
  it('renames an Alias through the shared Title interaction and preserves Target content', async () => {
    ...
    fireEvent.keyDown(screen.getByRole('textbox', { name: 'Thing title' }), { key: 'Enter' });
  ...
  it('opens each Alias on resolved Target content without a source editor', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Close Thing A again' }));

    fireEvent.click(await screen.findByRole('button', { name: 'Open Thing A once more' }));
    expect(await screen.findByText('A source')).toBeVisible();
    expect(screen.queryByRole('textbox', { name: /Markdown source/ })).not.toBeInTheDocument();
    expect(bodyOf(session, THING_ID)).toBe('A source');
    await settled(session);
  });

  /**
   * Escape cancels the body draft and returns the open Thing to rendered Markdown.
   */
  ...
  /**
   * Presenting draws the active Thing's content *in place of* the Thing
   * (`showActiveThingContent`), so a live editor cannot survive it and its draft
  ...
  /**
   * Add Thing finishes by putting a caret in the created Thing's title editor, and
   ...
   * created a Thing the author was never given the editor to name.
   */
  it('cannot add a Thing over a live content edit', async () => {
    const session = mount();
    await openEditor();

    const create = createThingControl();
    expect(unavailable(create)).toBe(true);
    fireEvent.click(create);

    expect(session.getState().working.things).toHaveLength(2);
    await settled(session);
  });

  /**
   * The Diagram's rename is withdrawn while a Thing title editor owns the caret;
   * its address is not.
   *
  ...
  it('adds a Thing again once the author has settled the edit', async () => {
    const session = mount();
    await openEditor();
    fireEvent.keyDown(screen.getByRole('textbox', { name: 'Markdown source of A' }), {
      key: 'Escape',
    ...
    expect(session.getState().working.things).toHaveLength(3);
    expect(await screen.findByRole('textbox', { name: 'Thing title' })).toHaveValue('Thing 1');
    await settled(session);
  });

  /**
   * The pane keeps its draft in `useState`, seeded once from the Thing it was
   * mounted on. Opening a second Thing without closing the first therefore had
   * the same React element in the same position — so the state survived while
   * `thing.id` changed underneath it, and the fields were now A's text wearing
   ...
   * author cannot see.
   */
  it('never carries one Thing’s draft onto another', async () => {
    const session = mount();
    await openEditor();
    replaceMarkdownSource('A rewritten');

    fireEvent.keyDown(screen.getByTestId(`rf__node-${OTHER_THING_ID}`), { key: 'Enter' });
    ...
    expect(thingTitleOf(session, OTHER_THING_ID)).toBe('B');
    expect(bodyOf(session, OTHER_THING_ID)).toBe('B source');
    await settled(session);
  });
  ...
  it('closes without committing when Escape is pressed outside the fields', async () => {
    ...
    fireEvent.click(screen.getByTestId(`rf__node-${OTHER_THING_ID}`));
    expect(screen.getByRole('textbox', { name: 'Markdown source of A' })).toBeVisible();
    fireEvent.keyDown(screen.getByRole('textbox', { name: 'Markdown source of A' }), {
      key: 'Escape',
    });
...
/*
 * Opening a Space Thing is deliberately not covered here.
 *
 * It used to be, as the one kind Opening refused — and that refusal is gone:
 * Opening is one Diagram-owned Edit that asks no question about the Thing's kind
 * (ADR 0064), and a Space Thing has something to draw Open, being the Diagram
 ...
 * `space-thing-selection.test.tsx` rather than being weakened to fit here.
 */
describe('the Thing affordance on the graph', () => {
  it('opens the Thing on rendered Markdown in place', async () => {
    const session = mount();

    fireEvent.click(await screen.findByRole('button', { name: 'Open Thing A' }));
    expect(await screen.findByText('A source')).toBeVisible();
    expect(screen.queryByRole('textbox', { name: 'Markdown source of A' })).not.toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Close Thing A' })).toBeVisible();
    await settled(session);
  });

  /**
   * No gesture on a Thing's body opens it (ADR 0036); the Title and Opening each
   * have their own explicit control.
   */
  it('is the only pointer graph in — the Thing body opens nothing', async () => {
    const session = mount();
    const thing = (await screen.findByRole('heading', { name: 'A' })).closest('.react-flow__node');
    if (thing === null) throw new Error('Thing A is not drawn as a node');

    fireEvent.click(thing);
    fireEvent.doubleClick(thing);

    // The authored Open state itself, and the control an Open Thing offers. The
    // Thing the gesture lands on is the evidence: a pane that is no longer built
    ...
    expect(screen.queryByRole('button', { name: 'Close Thing A' })).not.toBeInTheDocument();
    await settled(session);
  });
});
