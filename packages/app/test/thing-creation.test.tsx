...
import { createThing, presentControlBehindAModal, unavailable } from './command-dock';

/**
 * Creating Things, from the controls an author actually has.
 *
 ...
 * Space with no Things at all.
 */
const noThings: SpaceSnapshot = spaceSnapshotSchema.parse({
  id: SPACE_ID,
  document: {
    version: 1,
    title: 'Space',
...
function mount(value: SpaceSnapshot = snapshot): SpaceSession {
  ...
  mountSpace(
    { id: runtime(value).id, session, app: composeApp({ spaceSession: session }), spaceThings },
    (app) => {
...
const thingTitles = (session: SpaceSession) =>
  thingsOf(session).map((thing) => thing.document.title);
...
describe('Add Thing', () => {
  it('creates a neutrally titled Thing and opens its name editor', async () => {
    const session = mount();

    await readyToAuthor();
    createThing('Markdown Thing');

    expect(thingTitles(session)).toEqual(['A', 'B', 'Thing 1']);
    const created = thingsOf(session)[2]!;
    ...
    // editor arrives with the projection that first draws the created Thing.
    const input = await screen.findByRole('textbox', { name: 'Thing title' });
    expect(input).toHaveValue('Thing 1');
    expect(input).toHaveFocus();
    await settled(session);
  });

  it('renames the created Thing in place, and leaves focus on the Thing', async () => {
    const session = mount();
    await readyToAuthor();
    createThing('Markdown Thing');
    const input = await screen.findByRole('textbox', { name: 'Thing title' });
    ...
    expect(thingTitles(session)).toEqual(['A', 'B', 'Consequences']);
    const created = thingsOf(session)[2]!;
  ...
  /**
   * The Thing is already created by the time the editor opens — there is no
   * creation draft — so Escape cancels the *rename* and nothing else. Focus
   * still has to land on the Thing rather than on `<body>`.
   */
  it('keeps the Thing when its naming is cancelled', async () => {
    const session = mount();
    await readyToAuthor();
    createThing('Markdown Thing');
  ...
  /**
   * The created Thing is selected as well as named, and selected in the sense
   * React Flow means — not only in the sense Authoring does.
   *
   * The two are separate notions and the Thing is created between them: the
   * store's `selectedThingId` is set in the same tick as the Edit, one render
   ...
   * unselected and the two notions disagree for good — the Thing reads as
   * selected on screen while React Flow has no selected node at all. `F2` asks
   * React Flow, so it is what notices.
   */
  it('leaves the created Thing selected for F2, not only for authoring', async () => {
    const session = mount();
    await readyToAuthor();
    createThing('Markdown Thing');
    // Out of the naming editor creation opens, so `F2` is answered by the
    // canvas rather than typed into a field.
    const naming = await screen.findByRole('textbox', { name: 'Thing title' });
    fireEvent.keyDown(naming, { key: 'Escape' });
    const created = thingsOf(session)[2]!;
    const node = document.querySelector(`.react-flow__node[data-id="${created.id}"]`);
    if (node === null) throw new Error('the created Thing is not drawn as a node');

    fireEvent.keyDown(node, { key: 'F2' });

    const renaming = await screen.findByRole('textbox', { name: 'Thing title' });
    expect(renaming).toHaveValue('Thing 1');
    await settled(session);
  });
   ...
   */
  it('creates the first Thing of an empty Diagram as its only member', async () => {
    const session = mount(noThings);

    await readyToAuthor();
    createThing('Markdown Thing');

    const diagram = diagramsOf(session)[0]!;
    expect(thingTitles(session)).toEqual(['Thing 1']);
...
describe('Add Alias', () => {
  /**
   * The creation state is editor-local and nothing else (ADR 0042): no Thing, no
   * authored Thing and no commit. This is the package's own gate — cancelling before a
  ...
  it.skip('replaced by Dialog Escape-as-Cancel coverage', async () => {
    ...
    expect(screen.queryByTestId('new-alias')).not.toBeInTheDocument();
    expect(thingTitles(session)).toEqual(['A', 'B']);
    await settled(session);
  });
  ...
  it('opens on the Target picker, which searches non-Alias Things by title', async () => {
  ...
  it('keeps a title the author entered instead of the Target’s', async () => {
    ...
    fireEvent.click(screen.getByRole('option', { name: 'Markdown Thing A' }));

    expect(thingsOf(session)[2]?.document).toEqual({
      title: 'Recap',
      kind: 'alias',
      target: THING_ID,
    });
  ...
  /**
   * The created Alias is where the author continues, exactly as after Add Thing.
   *
   * Both creations end at the Thing they made, with its name editor open on it,
   * and both reach that through the one continuation `thing-creation.ts`
   ...
   * Thing takes the other branch — its title was typed on the pane, so it
   * returns to Add Thing with nothing to name.
   */
  ...
  /**
   * An Alias needs a Thing that owns its content, and a Space may not have one.
   *
   ...
   * filtered count is zero, and with no Thing registered that is true of every
   * search — so a hand-rolled explanation next to it would stack under "No Thing
   * matches that search" rather than replace it.
   */
  it('explains itself when the Space holds no eligible Thing', async () => {
    const session = mount(noThings);
    await openAliasCreation();
    fireEvent.keyDown(screen.getByRole('combobox', { name: 'Target' }), { key: 'ArrowDown' });
    ...
    expect(
      screen.getAllByText(
        'An Alias needs a Thing that owns its content, and this Space has none yet.',
      ),
    ).not.toHaveLength(0);
    expect(screen.queryByText('No Thing matches that search.')).not.toBeInTheDocument();
   ...
   */
  it('describes nothing once the Space holds a Thing to choose', async () => {
    const session = mount();
    await openAliasCreation();
  ...
  it('keeps the combobox pointing at a list that exists with nothing to list', async () => {
    ...
    const results = screen.getByTestId('thing-picker-results');
 ...
 * Navigation clears an opened Thing on its way in; the Alias creation state is
 ...
 *
 * The toolbar already knows this — its Add Thing control is disabled on
 * `creatingAlias` as well as on an opened Thing — and the canvas was told only
 * about the opened Thing. Both panes cover the graph the same way, so both have
 * to withdraw the same affordances.
 */
describe('the graph behind the Alias creation pane', () => {
  it('answers no Add Thing shortcut while the pane is open', async () => {
    const session = mount();
    await openAliasCreation();

    fireEvent.keyDown(screen.getByTestId(`rf__node-${THING_ID}`), { key: 'c' });
  ...
  /**
   * `openThingForEditing` declines a second open while a Thing is open, on the
   ...
   * request to discard it. Without this the Thing opened behind the pane, which
   * hid itself while a Thing was open and came back when it closed.
   */
  it('declines to open a Thing, leaving the pane exactly as it was', async () => {
    const session = mount();
    await openAliasCreation();
    fireEvent.change(screen.getByTestId('new-alias-title'), { target: { value: 'Recap' } });

    fireEvent.keyDown(screen.getByTestId(`rf__node-${THING_ID}`), { key: 'Enter' });

    expect(screen.queryByRole('button', { name: /^Close Thing / })).not.toBeInTheDocument();
    expect(screen.getByTestId('new-alias')).toBeVisible();
