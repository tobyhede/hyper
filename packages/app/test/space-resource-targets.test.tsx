...
const TARGET_MAP_ID = uuidSchema.parse('00000000-0000-4000-8000-000000000003');
const OTHER_SPACE_RESOURCE_ID = uuidSchema.parse('00000000-0000-4000-8000-000000000004');
const OTHER_TARGET_ID = uuidSchema.parse('00000000-0000-4000-8000-000000000005');
const OTHER_TARGET_MAP_ID = uuidSchema.parse('00000000-0000-4000-8000-000000000006');
...
/** The Resources a completed Edit hands the canvas: one Space Resource, a fresh array. */
...
/** The same canvas with a second Space Resource, pointed at a second target Space. */
const twoResources = (): readonly Resource[] => [
  ...resources(),
  {
    id: OTHER_SPACE_RESOURCE_ID,
    title: 'Also nested',
    kind: 'space',
    spaceId: OTHER_TARGET_ID,
    map: OTHER_TARGET_MAP_ID,
 ...
 * is the ordinary mounting sequence rather than a test-only shape. A Space Resource
 ...
 * Resource on the canvas draws without its target for the life of the page.
 */
describe('reading the Spaces a canvas references', () => {
  it('answers a target present at mount, through StrictMode’s double mount', async () => {
    ...
    render(
      <StrictMode>
        <Probe read={read} resources={resources()} />
      </StrictMode>,
    );
   ...
   * timeout. Recording the set as read regardless would leave every Space Resource
  ...
  it('asks again after a read that rejected', async () => {
    ...
    // Resources array, and this is the render that finds the set still unread.
    view.rerender(<Probe read={read} resources={resources()} />);

    await waitFor(() => expect(screen.getByTestId('titles')).toHaveTextContent('Architecture'));
  });

  /**
   * One unavailable target does not blank the Space Resources pointed elsewhere.
   *
   ...
   * says nothing about the one the Resource beside it points at. Failing the batch
   * would hold every Space Resource on the canvas without its title and selectors
  ...
  it('keeps the targets that were read when another target’s read rejects', async () => {
    ...
    render(<Probe read={read} resources={twoResources()} />);
   ...
   * previous answer is what the Resource keeps drawing meanwhile — the alternative
   * is a Resource blanking because a *different* Space was edited.
   */
  it('carries a target already read through a batch its own read fails in', async () => {
    ...
    // A second Space Resource is a new set of referenced Spaces, so both are read
    // again — and this time the first target's Space cannot be reached.
    firstTargetFails = true;
    view.rerender(<Probe read={read} resources={twoResources()} />);
