...
test(
  ...
  async ({ page }) => {
    ...
    const actions = resource.getByRole('toolbar', { name: 'Resource A', exact: true });
    const resize = resource.locator('.rf-resource-node__resize-control');
    for (const operation of ['Open', 'Close']) {
      await resource.hover();
      await resource.getByRole('button', { name: `${operation} Resource A`, exact: true }).click();
      await page.mouse.move(1, 1);
      await expect(actions).toHaveCSS('opacity', '0');
      await expect(actions).toHaveCSS('pointer-events', 'none');
      if (operation === 'Open') {
        await expect(resize).toHaveCSS('opacity', '0');
        await expect(resource.locator('.rf-resource-node__resize-mark')).toHaveCSS('opacity', '0');
      } else {
    ...
    const open = resource.getByRole('button', { name: 'Open Resource A', exact: true });
    await open.focus();
    await expect(actions).toHaveCSS('opacity', '1');
    await open.press('Enter');
    const close = resource.getByRole('button', { name: 'Close Resource A', exact: true });
...
/**
 * The shared surface has to *fit* the room a Resource gives it — which is the one
 * detail about mounting the Dock's strip on a Resource that could go wrong quietly.
 *
 * A Resource is 260 canvas units wide with `overflow: hidden`, and its rail is
 ...
 */
test('the revealed commands fit inside their Resource, Closed, Open and zoomed out', async ({
  page,
}) => {
  await page.goto('/');
  const resource = nodeByTitle(page, 'A');
  ...
  /** The strip's box, its ring-inclusive focused control's box, and the Resource's. */
  ...
  await resource.getByRole('button', { name: 'Open Resource A', exact: true }).click();
  await settled(page);
  await resource.hover();
  await fits();
