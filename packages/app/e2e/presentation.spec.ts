...
test('walks a presentation path with keyboard navigation', async ({ page }) => {
  await page.goto('/');
  ...
  // Select a path and enter presentation mode.
  await page.getByTestId('path-selector').click();
...
test('offers more than one named path', async ({ page }) => {
  await page.goto('/');
...
test('shows the selected path as a single colored flow', async ({ page }) => {
  await page.goto('/');

  // A legend maps each path to a color.
  await expect(page.getByTestId('path-legend').locator('.legend__item')).toHaveCount(2);

  // The default "main" path: 6 cards, 5 rail edges, 10 ports (in/out per step).
  await page.getByTestId('path-selector').click();
  await page.getByRole('option', { name: 'Main walkthrough' }).click();
  ...
  // Rails are colored (an edge path carries a non-empty stroke color).
...
test('switching the path swaps the visible flow', async ({ page }) => {
  await page.goto('/');
