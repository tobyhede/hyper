...
export async function exerciseSpaceThingEntityMenu(page: Page, thing: Locator): Promise<void> {
  ...
  expect(
    await typography((await thingControls(page, node)).getByTestId('space-thing-diagram')),
  ).toEqual(await typography(page.locator('[data-testid="selected-canvas"]:visible')));
  expect(
    await typography((await thingControls(page, node)).getByTestId('space-thing-graph')),
  ).toEqual(await typography(page.locator('[data-testid="active-graph"]:visible')));
  const menu = async () => {
    const trigger = (await thingControls(page, node)).getByRole('button', {
  ...
  await expect(node.getByRole('button', { name: /^Enter/ })).toHaveCount(0);
  await page.getByRole('menuitem', { name: 'Rename', exact: true }).click();
  const title = page.getByRole('textbox', { name: 'Thing title', exact: true });
  await expect(title).toBeFocused();
  await title.fill('Space card');
  await title.press('Enter');
  await expect(node.getByRole('heading', { name: 'Space card', exact: true })).toBeVisible();
  await menu();
