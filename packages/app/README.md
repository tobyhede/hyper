...
(ADR 0020) and every `.md` beside its space file is a resource, so a `README.md` in
...
holding structure — `version`, `id`, `title`, `maps` and `defaultMap` —
and one markdown file per resource, either beside it or under `resources/`. The Meta
Space uses both locations so the two-location scan is exercised by the Space
the app actually opens.

`defaultMap` is the opening canvas (ADR 0079). A selected Map draws only
...
Meta holds three Maps: two disconnected collections that share no Resources —
one of them the opening canvas, with several Graphs over a shared spine and a
member no Edge reaches — and a third Map of Space Resources. That third
Map is how ordinary Spaces hang off Meta: a converging reference, depth
...
Map. Nothing renders it, so its positions are a plain deterministic grid.

The fixture's shape exists to exercise the behaviours the e2e suite covers:

- **Multiple graphs over a shared spine.** Several Graphs share Resources. Each
  Resource declares four shared anchors, one on each side — graph-independent
...
- **A Map member no Edge reaches.** Placed on the opening Map, on none
  of its Graphs — the state Add Resource and the Resources list both author.
- **Linked Spaces.** Space Resources on Meta, with a converging reference and
  depth three, so Enter, switching and presentation have ordinary Spaces to
  reach.

Graphs here are **lines**: each non-terminal Resource has one Edge out. That is the
