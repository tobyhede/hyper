...
describe('ViewSelector', () => {
  it('names the current View and offers only the shipped Views', async () => {
    ...
    // The label truncates, so the title is what keeps the whole value readable.
    expect(trigger).toHaveAttribute('title', 'Choose view · Flow');
  ...
  it('stays controlled across the trigger the toolbar keeps out of React Flow', () => {
    render(<ViewSelector value="grid" active onValueChange={() => undefined} />);

    // A Select trigger is a focusable `button` that no input tag excludes, and
    // React Flow subscribes its delete keys on `document`.
    expect(screen.getByRole('combobox', { name: 'Choose view' })).toHaveClass('nokey');
  });
});
