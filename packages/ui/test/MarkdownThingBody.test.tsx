...
import { MarkdownThingBody } from '../src/MarkdownThingBody';
...
const body = (props: Partial<Parameters<typeof MarkdownThingBody>[0]> = {}) => (
  <MarkdownThingBody
...
/**
 * The body in the Thing that mounts it, which is the only place the two ends of
 * an edit are drawn: this surface publishes them and the Thing's rail draws them
 * (`thing-content-edit.ts`). Mounted alone, the body keeps its keys and offers no
 * control of its own.
 */
const onThing = (props: Partial<Pick<Parameters<typeof MarkdownThingBody>[0], 'editor'>> = {}) => (
  <CanvasThing
    front={
...
describe('MarkdownThingBody', () => {
  ...
  it('renders no body-level edit control', () => {
    ...
    const surface = container.querySelector('.markdown-thing-body');
  ...
  it('keeps rendered Markdown click-to-edit without drawing an icon', () => {
    ...
    const target = screen.getByTestId('markdown-thing-body-edit-target');
  ...
  it.each([
    ...
    await source();
    const hint = container.querySelector('.markdown-thing-body__shortcut-hint');
    ...
    expect(
      Array.from(hint.querySelectorAll('.markdown-thing-body__shortcut')).map(
  ...
  it('offers no control of its own when nothing published a rail to draw one on', async () => {
    ...
    expect(screen.queryByRole('button', { name: /^Save Thing/ })).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: /^Cancel editing Thing/ })).not.toBeInTheDocument();
  });

  it('takes the same two exits from the rail as from its own keys', async () => {
    const editor = { onComplete: vi.fn(), onEnd: vi.fn() };
    render(onThing({ editor }));
    await source();
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Cancel editing Thing Strategies' }));
    expect(editor.onComplete).not.toHaveBeenCalled();
    expect(editor.onEnd).toHaveBeenCalledOnce();

    await source();
    fireEvent.click(screen.getByRole('button', { name: 'Save Thing Strategies' }));
  ...
  it('withdraws those two ends from the rail when the caret goes back', async () => {
    const { rerender } = render(onThing({ editor: { onComplete: vi.fn(), onEnd: vi.fn() } }));
    await source();
    expect(screen.getByRole('button', { name: 'Save Thing Strategies' })).toBeVisible();

    rerender(onThing());

    expect(screen.queryByRole('button', { name: 'Save Thing Strategies' })).not.toBeInTheDocument();
    expect(
      screen.queryByRole('button', { name: 'Cancel editing Thing Strategies' }),
    ).not.toBeInTheDocument();
  });

  it.each([['Save Thing Strategies'], ['Cancel editing Thing Strategies']])(
    ...
    async (action) => {
      ...
      expect(screen.getByRole('button', { name: 'Edit Thing Strategies' })).toHaveFocus();
    },
  );
  ...
  it('withholds the React Flow escape hatches until there is a caret to protect', () => {
    ...
    // At rest the rendered Markdown is text on a Thing, and dragging by it is what
    // an author expects.
    ...
    // everywhere, so no Expanded Thing is a hole to wheel-pan across (ADR 0064).
