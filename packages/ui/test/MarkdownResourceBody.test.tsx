...
const body = (props: Partial<Parameters<typeof MarkdownResourceBody>[0]> = {}) => (
...
/**
 * The body in the Resource that mounts it, which is the only place the two ends of
 * an edit are drawn: this surface publishes them and the Resource's rail draws them
...
describe('MarkdownResourceBody', () => {
  ...
  it('renders no body-level edit control', () => {
    ...
    const surface = container.querySelector('.markdown-resource-body');
  ...
  it('keeps rendered Markdown click-to-edit without drawing an icon', () => {
    ...
    const target = screen.getByTestId('markdown-resource-body-edit-target');
  ...
  it.each([
    ...
    await source();
    const hint = container.querySelector('.markdown-resource-body__shortcut-hint');
    ...
    expect(
      Array.from(hint.querySelectorAll('.markdown-resource-body__shortcut')).map(
  ...
  it('offers no control of its own when nothing published a rail to draw one on', async () => {
    ...
    expect(
      screen.queryByRole('button', { name: /^Cancel editing Resource/ }),
    ).not.toBeInTheDocument();
  });

  it('takes the same two exits from the rail as from its own keys', async () => {
    const editor = { onComplete: vi.fn(), onEnd: vi.fn() };
    render(onResource({ editor }));
    await source();
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Cancel editing Resource Strategies' }));
    expect(editor.onComplete).not.toHaveBeenCalled();
    expect(editor.onEnd).toHaveBeenCalledOnce();

    await source();
    fireEvent.click(screen.getByRole('button', { name: 'Save Resource Strategies' }));
  ...
  it('withdraws those two ends from the rail when the caret goes back', async () => {
    const { rerender } = render(onResource({ editor: { onComplete: vi.fn(), onEnd: vi.fn() } }));
    await source();
    expect(screen.getByRole('button', { name: 'Save Resource Strategies' })).toBeVisible();

    rerender(onResource());

    expect(
      screen.queryByRole('button', { name: 'Save Resource Strategies' }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole('button', { name: 'Cancel editing Resource Strategies' }),
    ).not.toBeInTheDocument();
  });

  it.each([['Save Resource Strategies'], ['Cancel editing Resource Strategies']])(
    'returns focus to Edit after leaving through %s',
    async (action) => {
      render(<EditingResource />);
      await source();

      fireEvent.click(screen.getByRole('button', { name: action }));

      expect(screen.getByRole('button', { name: 'Edit Resource Strategies' })).toHaveFocus();
    },
  );
  ...
  it('withholds the React Flow escape hatches until there is a caret to protect', () => {
    ...
    // At rest the rendered Markdown is text on a Resource, and dragging by it is what
    // an author expects.
