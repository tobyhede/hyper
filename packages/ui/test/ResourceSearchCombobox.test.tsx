...
const CHOICES: readonly ResourceChoice[] = [
  { id: RESOURCE_A, title: 'Alpha', kind: 'markdown' },
  { id: RESOURCE_B, title: 'Beta', kind: 'markdown' },
  { id: RESOURCE_C, title: 'Gamma', kind: 'markdown', refusal: 'That Edge already exists.' },
];

/** A Resource whose Title runs to more than one line (ADR 0083). */
const LADDERED: readonly ResourceChoice[] = [
  { id: RESOURCE_A, title: 'Auth\nHow a session begins\nOAuth only', kind: 'markdown' },
  { id: RESOURCE_B, title: 'Beta', kind: 'markdown' },
];
...
describe('ResourceSearchCombobox', () => {
  /**
   * One visible input names the field and shows what it currently holds, so an
   * Edge editor drawn over the canvas says which Resource each endpoint is without
   * opening anything.
   */
  it('names the field and shows the chosen Resource before it is opened', () => {
    render(
      <ResourceSearchCombobox
        label="To"
        choices={CHOICES}
   ...
   * would leave an author searching for a Resource the list simply does not show;
  ...
  it('answers the chosen Resource', () => {
    const onValueChange = open();

    fireEvent.click(screen.getByRole('option', { name: 'Markdown Resource Beta' }));

    expect(onValueChange).toHaveBeenCalledWith(RESOURCE_B);
  });

  /**
   * The search matches the **title**, not the value behind it. Every row's id is
   * a Resource's UUID, so a filter over the value would rank a one-letter search by
   * hex noise.
   */
  ...
  it('describes an empty list on the field itself', () => {
    render(
      <ResourceSearchCombobox
        ...
        value={null}
        onValueChange={() => undefined}
        emptyMessage="This Space holds no other Resource that owns its content."
      />,
    );
    ...
    expect(document.getElementById(described[0] ?? '')).toHaveTextContent(
      'This Space holds no other Resource that owns its content.',
    );
  });

  /**
   * The empty-list note is an addition, never a replacement. A caller's own
   * description is a refusal attached to this field — a Reference Resource whose Target has
   * left the Space is refused *and* has no Resource left to choose, so both are true
  ...
  /** With Resources to offer, the field carries only what the caller asked for. */
  ...
  /**
   * The one surface where a Resource's name and its Title come apart (ADR 0083).
   * An author's recall does not respect which line they typed a word on, so a
   * word from a subtitle finds the Resource — while the row and the field still say
   * the name, because a newline in a row draws as a broken-looking label.
   */
  describe('a Title written on more than one line', () => {
    it('shows the Resource’s name in the field and in the row', () => {
      render(
        <ResourceSearchCombobox
          label="Target"
          choices={LADDERED}
      ...
      expect(screen.getByRole('option', { name: 'Markdown Resource Auth' })).toBeInTheDocument();
    });

    it('finds the Resource by a word from a line below the name', () => {
      render(
        <ResourceSearchCombobox
          label="Target"
          choices={LADDERED}
