...
import './markdown-resource-body.css';

/**
 * What ends a body edit. Its presence on {@link MarkdownResourceBodyProps} *is* the
 * caret, so a caller cannot ask for one without saying what commits it and what
 * takes the caret back — the pairing `CanvasResource` already makes for its own
 * title editor and `ResourceNodeData` for the adapter's.
 */
export interface MarkdownResourceBodyEditor {
  /**
   ...
   *
   * Deliberately not `onCancel`, which is what `ResourceTitleEditor` calls the
...
export interface MarkdownResourceBodyProps {
  /** The Markdown this Resource owns, as bytes. */
  readonly source: string;
  /** Names the writing surface for a screen reader, e.g. the Resource's title. */
 ...
 * command; the surrounding Resource rail owns the only visible Edit action. While
 ...
 * Resource's title editor, which does complete on blur: a title is one refusable
 ...
 *
 * Those same two exits are published to the surrounding Resource through
 * `ResourceContentEdit`, which draws them on its rail in place of the control that
 * began this edit. They are published rather than drawn here because the rail is
 * the Resource's — this surface reaches the paper's edges and has no corner of its
 ...
 * dragging the Resource out from under the caret, and `nopan` stops a click-drag
 ...
 * nothing here knows React Flow exists, exactly as `canvas-resource.css` does not.
 */
export function MarkdownResourceBody({
  source,
  ariaLabel,
  onBeginEdit,
  editor,
  ...
  // Each edit starts from what the Resource currently holds. Adjusted during render
  ...
  /**
   * The two exits, as operations the surrounding Resource can draw.
   *
   * Built once and never rebuilt, so the Resource is not re-rendered on every
  ...
  const exits = useMemo<ResourceContentEdit>(
    () => ({
  ...
  const leave = (commit: boolean): void => {
    ...
    // failure this is here to prevent. Both are needed because a caller may keep
  ...
  // Resource's rail reads the presence of an edit from the same fact this surface
  ...
  return (
    <div
      className={cn(
        ...
        editing && 'markdown-resource-body--editing nodrag nopan',
        className,
      )}
      onKeyDown={onKeyDown}
    >
      {editing ? (
        <>
          <Suspense
            fallback={<div className="markdown-resource-body__pending" aria-hidden="true" />}
          >
            <MarkdownSourceEditor
              key={generation}
              ref={receiveEditor}
              className="markdown-resource-body__source"
              value={draft}
              ariaLabel={ariaLabel}
          ...
          <div className="markdown-resource-body__shortcut-hint" aria-hidden="true">
            <span className="markdown-resource-body__shortcut">
              <KbdGroup className="markdown-resource-body__shortcut-keys">
        ...
        <>
          <RenderedMarkdown
            markdown={source}
            className="markdown-resource-body__rendered resource__body"
          />
          {onBeginEdit !== undefined && (
            <Button
              variant="ghost"
              className="markdown-resource-body__edit-target"
              data-testid="markdown-resource-body-edit-target"
