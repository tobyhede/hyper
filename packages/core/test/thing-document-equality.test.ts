...
import { markdownThingDocumentSchema, markdownThingSchema } from '../src/index';

/**
 * The stored document is the whole markdown Thing less its id, and something has
 * to say so.
 *
 * `markdownThingDocumentSchema` is written as `markdownThingSchema.omit({ id:
 ...
 * re-declared beside the thing schema, or one of them gains a rule the other
...
describe('a stored markdown document is the thing less its id', () => {
  ...
  it('drops the id and keeps every other field', () => {
    expect(Object.keys(markdownThingDocumentSchema.shape).sort()).toEqual(
      Object.keys(markdownThingSchema.shape)
        .filter((key) => key !== 'id')
        .sort(),
    );
  });
   ...
   * `markdownThingSchema` along the path this file exists to guard. `omit`
   ...
   * refuses.
   */
  it('shares one instance of every rule with the thing schema', () => {
    const thingFieldSchemas: ZodRawShape = markdownThingSchema.shape;
    for (const [field, schema] of Object.entries(markdownThingDocumentSchema.shape)) {
      expect(schema, `the document's "${field}" is not the thing's`).toBe(thingFieldSchemas[field]);
    }
  });
   ...
   * a Thing's Title normalizes and must keep one non-empty line, which is what
  ...
  it('accepts a document exactly when the thing accepts it with an id', () => {
    ...
    for (const title of titles) {
      for (const body of bodies) {
        for (const kind of kinds) {
          ...
          examined += 1;
          const asDocument = markdownThingDocumentSchema.safeParse(document);
          const asThing = markdownThingSchema.safeParse({ ...document, id: THING_ID });
          if (asDocument.success !== asThing.success) {
            disagreements.push(
              `${JSON.stringify(document)}: document ${asDocument.success}, thing ${asThing.success}`,
            );
            continue;
          }
          if (asDocument.success && asThing.success) {
            expect(asThing.data).toEqual({ ...asDocument.data, id: THING_ID });
          }
        }
      }
    }
