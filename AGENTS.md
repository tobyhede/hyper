...
- **A Layout is a strategy, and the contract lives in `graph`, not the adapter.** `Layout` is `(graph: LayoutGraph) => Promise<LayoutGraph>`: geometry lands as optional fields on the cards and handles, mirroring how ELK itself models it. There is no `Arrangement` result type — see ADR 0005. `gridLayout` (pure, places no handles) exists partly to keep the seam honest; if a change only works for `elkLayout`, the contract has leaked. Which cards get arranged is the view's choice, passed in. The contract is uniformly async — the earlier `LayoutGraph | Promise<LayoutGraph>` union was collapsed once nothing exercised the sync branch (`gridLayout` is now `async`); see `.scratch/layout-seam/issues/06-revisit-async-optionality.md`.
