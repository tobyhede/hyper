...
- [x] Tracked codemod, not a `sed` one-liner, under this directory. Read the
      ...
      must not open.
- [x] Schema, fixtures, refusals, Create Reference, kind glyph, accessible
      names (`Reference Thing`, `Reference to a Markdown Thing`,
      `Reference to a Space Thing`).
- [x] `SpaceReferenceError`, `validateReferences`, `referenceableSpaces`, and
      Space Thing “reference” prose are not this kind. Do not rename them.
- [x] `test/unit/current-domain-vocabulary.test.ts` gains a block that reports
...
- [x] Merge with `-M20%`. Check the replay rather than trusting a `--dry`.
- [x] `pnpm verify` on the finished state. `pnpm e2e` and `pnpm e2e:ladle`
