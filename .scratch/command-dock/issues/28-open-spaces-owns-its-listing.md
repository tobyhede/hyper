...
5. **`select(spaceId)`** on `OpenSpaces`, named for `CONTEXT.md`'s "Selecting an entry switches to that Space". It switches to an open Space, opens a closed Meta with no Opener, and refuses anything else. It returns an outcome — `{ kind: 'switched' } | { kind: 'opened' } | { kind: 'refused', code: 'space-not-open' }` — each carrying the title the listing showed. Load failures still throw. Selecting the Space already on the canvas answers `switched`.
...
10. **A refusal reads like a failure.** `space-not-open` only happens in a race (the Space closed between render and click); the Dock says "<Title> could not be opened." for it as for a thrown failure, with the title from the outcome.
