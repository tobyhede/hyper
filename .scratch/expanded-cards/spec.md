...
The Card must hold no second opinion about its own size **inside a node**. A `.canvas-card` that declares the collapsed constant and overrides it to `100%` while Expanded is a passenger that *leaves* the transition the instant the Expanded flag flips — which is precisely what the review prototype does, and why it animates on open and snaps on close: one frame after a close the node wrapper is still 560x420 while the Card inside is already 260x146, easing a box nothing is attached to while the neighbours glide home.

**The rule is scoped to the node, not to the component.** `CanvasCard` is also mounted outside any sized box, and not only by stories: `NewCardPreview` draws the production ghost in a `ViewportPortal` div that sets a width and no height. A blanket `.canvas-card { width: 100%; height: 100% }` collapses that ghost to title height, and `min-width`/`min-height` cannot rescue it — they floor a box, they do not cap `100%` in a shrink-to-fit parent, and `height: 100%` of an auto-height parent is still nothing. That is the reason already written into `canvas-card.css`, and it stands.

So the collapsed default stays declared on `.canvas-card`, and the node branch overrides it unconditionally — `.rf-card-node__inner > .canvas-card { width: 100%; height: 100% }`. `.rf-card-node__inner` itself becomes `100%`/`100%` unconditionally too, and loses its own `[data-expanded]` geometry rule: it carries the identical discontinuity today, and collapsed it declares no height at all, so leaving it keyed on the flag just moves the snap one element down. `data-expanded` then governs content only: order, borders, what is rendered. Never geometry, on either element.

Two things this rule does **not** reach, both of which stay discontinuous and should be stated rather than discovered. `.rf-card-node__content` writes the collapsed constant a third time for the presenting branch, which draws `CardContent` and not `CanvasCard` at all. And Edge geometry snaps in both directions regardless: handle bounds and node positions are published to React Flow's store the moment the projection commits, and an SVG path `d` is not a transitionable property — so the Edges reach their final endpoints on frame one while the box eases. Fixing the box makes that snap *more* visible, not less.
...
**The lazy split is ui-owned (ADR 0067, refining ADR 0063).** `MarkdownSourceEditor` is reached through `packages/ui/src/markdown-source-editor-lazy.ts`, beside `MarkdownCardBody`, and `app` does not reach `@project/ui/MarkdownSourceEditor` at all. The ESLint zone is therefore a plain bar with no exception. `test/unit/codemirror-encapsulation.test.ts` scans the `ui` source tree, permits only the lazy module's dynamic import, and detects wrapped or multiline static value imports so the CodeMirror stack cannot return to the initial bundle unnoticed.
