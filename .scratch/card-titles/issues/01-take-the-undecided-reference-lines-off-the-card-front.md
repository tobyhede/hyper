...
`986fdde9`, with `900a4eec` completing it.
...
Three specs in `packages/app/e2e/editing.spec.ts` still asserted the copy (`toHaveValue('B')`, `nodeByTitle(page, 'B')).toHaveCount(2)`) and their prose still explained it. This ticket named `overview.spec.ts` and `space-card.spec.ts` and not this file, so they were missed until `pnpm e2e` ran on the integrated branch. Fixed in `900a4eec`, along with a stale comment in `space-authoring.ts` that still described the fallback.
