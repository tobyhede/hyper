...
| Arrange Cards through Layouts | Partial | Authored placement, Move, Open/Close/Resize and Layout-local state persist. The application does not expose complete Add/Rename/Select/Delete management. The working tree contains an uncommitted Layout-only ADR/ticket set, so it is a pending proposal rather than durable ownership this audit can assign. | Tracked ticket `v1-release/04` remains the owner until the Layout-only proposal is committed or rejected; its current conversion contract would then need reconciliation. |
...
| Use durable product URLs | Partial | Space, current Space View, Card, Graph and single-Space presentation destinations, History behavior and HTTP 400/404 policy are built. Space Card authoring and Enter/Open remain open. The uncommitted Layout-only proposal would replace the current Space View URL model if it lands. | Tracked owners are `entity-url-addressability/07–08`; the Layout-only proposal has no durable owner until its tracker is committed. |
| Export the complete authored result | Partial | Deterministic CLI export exists for one Space and tracks its exported revision. It does not write one Meta-rooted aggregate. | `v1-release/08`. Architecture issue `13` overlaps the aggregate read/commit seam; an uncommitted Layout-only ticket proposes additional format work but is not yet an owner. |
...
| Import and recover the same authored result | Partial | Single-Space/directory import and destructive truncation exist, but not versioned `hyper.json` aggregate intake, exact Meta identity or complete cross-Space validation. | `v1-release/08`; an uncommitted Layout-only ticket proposes additional format work but is not yet an owner. |
...
- **Layout ownership is pending outside the tracked release graph.** The tracked
  `v1-release/04` asks Create Layout to copy a Computed View. The working tree's
  uncommitted ADR 0076 and `layout-only-v1/01–06` instead remove Computed Views
  and make Add Layout empty. They cannot own release work until committed. Land
  that decision/tracker first or reject it and retain ticket 04; only then can
  the Definition of Done and affected URL/Space Card tickets be reconciled.
...
2. **Settle and track the Layout-only proposal.** Commit its ADR and tracker as a
   prerequisite, then retire Computed View/Space View requirements from
   `v1-release/04`, the Definition of Done and affected Space Card/URL tickets;
   or reject it and continue from the tracked conversion contract. Ticket 11
   cannot base a checkpoint on uncommitted ownership.
