...
Tags: release/v1
Blocked by: 03, 08, 16, 17; `entity-url-addressability/08`; `space-cards/10`
...
Two of this rehearsal's blockers were human decision gates rather than build
work when this note was written. Both have since landed, and neither is a gate
now:

- [V1/01](01-establish-the-meta-space-lifecycle.md) is `resolved`. Its criteria
  are built and its one open criterion is ticket 08's, so it left this ticket's
  `Blocked by:` line with it.
- [V1/17](17-preserve-structured-aggregate-refusals.md) is `ready-for-agent`.
  Its criteria 1 and 5 contradicted each other; the decision kept the
  one-sentence refusal for V1 and narrowed criterion 5 to the evidence that is
  genuinely missing. It remains a blocker, but as build work rather than a
