...
- For a Space Card Target, render its selected Space View and Graph without controls that change either selection.
...
- Reuse the Markdown and Space Card content renderers; do not introduce Alias-specific copies.
...
- [ ] An Alias targeting a Space Card renders the Target Card's selected Space View and Graph read-only under the Alias Title.
- [ ] No Open Alias can edit Markdown source, Space View selection, Graph selection or Target.
...
- [ ] Stable stories and Ladle E2E cover read-only Markdown and Space Card Target presentations.
- [ ] `pnpm verify`, `pnpm e2e` and `pnpm e2e:ladle` pass.

## Blocked work

The Space Card Target branch becomes executable when ADR 0068's Space Card is built. Its read-only capability and renderer reuse are part of this interface now so the later kind does not invent a second Alias model.
