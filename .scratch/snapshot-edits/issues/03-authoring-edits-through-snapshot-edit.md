...
- [ ] Authoring's `opened-thing`, `closed-thing`, `resized-thing`, `added-thing-to-diagram`, `removed-thing-from-diagram`, `deleted-thing`, `created-thing`, `created-alias` and `create-and-connect` arms call the module and map its refusal codes into `AuthoringRefusal`. `space-thing-deletion-unsupported` and the Alias Target check on creation stay in Authoring.
