...
**No shadow token is added, deliberately.** The scan above counted three black-based shadows, but two of those three are text inside a comment describing a value that was already replaced, and the scan did not exclude comments. Corrected, there is one live elevation shadow written in black — `SelectedEdgeControls`, `shadow-[0_6px_20px_rgb(0_0_0/45%)]` — and one 1px `drop-shadow` outline on a glyph, which is a legibility technique and not an elevation. See `.scratch/structural-tokens/issues/10-a-dark-theme-shadow-survives-on-one-surface.md`. Minting a token for the one live defect would make it permanent by naming it, so it stays until that ticket decides what replaces it.
