...
- [ ] The Dock's Graph list and an Open Space Resource's Graph choice draw a colour line per row, in that Graph's colour, and no Graph glyph
- [ ] The HUD key draws the same shared mark, unchanged in appearance
- [ ] The Dock's Graph identity and the Colour… item still draw the coloured Graph glyph
- [ ] Inventory entry, story and its Ladle and application proofs; `pnpm ui:catalog:check`, `pnpm e2e` and `pnpm e2e:ladle` green
