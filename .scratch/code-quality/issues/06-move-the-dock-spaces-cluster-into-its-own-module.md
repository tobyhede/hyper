...
- [x] The cluster's module exports one component the Dock mounts. Any context it shares with other clusters lives in a shared Dock module, not in the Dock file (`CommandDockSpaces.tsx` exports `SpacesControl` plus its prop types; contexts, hooks and constants in `command-dock-shared.ts`; shared pieces — `Divider`, `IdentitySurface`, `SetTrigger` — in `CommandDockParts.tsx`)
