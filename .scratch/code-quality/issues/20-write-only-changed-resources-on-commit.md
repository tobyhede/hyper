...
- *Still open, not this ticket's:* a commit that falls into that window still overwrites the recreated Space with its own snapshot, unvalidated against the replacing aggregate. That is how the fast path already behaved before this ticket, because it trusts the revision for the document it writes. Closing it would take a row version a recreation cannot repeat.
