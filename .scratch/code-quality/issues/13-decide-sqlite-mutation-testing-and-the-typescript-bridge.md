...
- [x] Each has a recorded decision. Every removal comes with a removal ticket, and with an ADR superseding the one that adopted it where an ADR did
...
Decision: **Keep, as an optional targeted diagnostic outside mandatory CI gates** (Toby Hede, 2026-09-25; recorded in `../DECISIONS-2026-09-25.md`). Since this evidence was gathered, #283 closed both findings tickets (mutation 05 and 06), so the "findings sit untriaged" argument no longer holds. Running it in CI was also decided that day and became ticket 25, which Toby Hede then deferred. Picking 25 up would reverse the standing "local diagnostic, never a gate" rule in AGENTS.md and `build-tooling.md`, so it would need an ADR.
