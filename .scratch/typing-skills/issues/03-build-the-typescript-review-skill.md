...
- [ ] **Exempt `tools/typing-fixtures/must-fail/` from the blocking list**, and say why in the skill rather than only here. Those seven files each carry exactly one construct from the list above, on purpose, and the ordinary `eslint`/`oxlint` runs already ignore them. A skill built to the bullet above reports every one of them as a defect the first time it is run over this repository, and the fix an agent then reaches for is deleting ADR 0062's evidence. The constructs stay reportable everywhere else, `must-pass/` included.
