...
**Delivery:** The decision is resolved on the donor. Its ADR and operational
integration remain to be extracted with the Ladle-infrastructure work before
Issue 08 treats the gate as available on `main`.
...
ADR 0052 makes stable Ladle stories production-parity evidence. They render the
unchanged exported production component through the smallest coherent boundary
that owns the claimed behavior. Harnesses may supply environment and public
inputs but may not replace state translation, lifecycle, focus, interaction or
framework geometry. Story-only modes and visual facsimiles are forbidden from
stable stories; unreachable and unresolved work belongs under `stories/review`.
...
The ADR records the rejected application catalogue route and substitute-story
...
seam. `AGENTS.md`, `$shadcn-first-ui` and `.ladle/config.mjs` now point to ADR
0052 at their respective operational boundaries rather than owning competing
rationales.
