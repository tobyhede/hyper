...
`packages/http/src/backend.ts`, where every arm fills them from the wire: `:182`,
`:188`, `:199`, `:206`, `:211` and `:217` set them to `problem.detail` — the
server's sentence — and the catch arms (`:147`, `:153`, `:158`) set them from a
thrown `Error`'s own message or a transport-owned literal. ADR 0057 rejected
`{ message: string }` by name for exactly this. The seven codes those two
failures actually carry — `network`, `timeout`, `unavailable`, `rate-limited`,
`invalid-commit`, `forbidden`, `protocol` — have no copy table at all.
...
placed still: it is a display sentence declared in
`packages/http/src/backend.ts:31` and exported from `@project/http`
(`packages/http/src/index.ts:37`), which the transport also reads itself as the
message for a thrown non-`Error` (`backend.ts:158`). Outside the package,
`packages/app/stories/surfaces/cards-drawer.stories.tsx:4` imports it and `:152`
builds a failure from it.
...
returns are hand-written sentences (`:1418`, `:1422`) — the second joins
...
**The offending Card ids stay in the sentence the author reads.**
`packages/app/test/SpaceApp.test.tsx:216` asserts the missing id appears in
`persistence-remote-refused`'s own text, and `:211-213` records why it is there
rather than in a `title` attribute: `role="alert"` announces what it contains,
and a reason a pointer has to hover to reach is one a keyboard or touch user
never gets. So the typed context has to reach the rendered copy, not only a
diagnostic.
...
own display copy. Its one internal use — `backend.ts:158`, the message for a
thrown non-`Error` — goes wherever that `message` field goes, rather than being
re-homed as a sentence somewhere else.

## Blast radius

Three tests pin the prose this change replaces, and one of them is Vitest — so
this does not land green on `e2e:ladle` alone:

- `packages/app/test/SpaceApp.test.tsx:215` asserts `persistence-remote-refused`
  has text content `'The remote space is invalid and was not accepted'`, and
  `:216` asserts the offending card id is in that same alert text. `pnpm verify`
  runs it.
- `packages/app/ladle-e2e/issue-14-space-sidebar.spec.ts:335-336` asserts
...
- `packages/app/ladle-e2e/cards-drawer.spec.ts:84` asserts the notice contains
  `NETWORK_FAILURE_MESSAGE`, imported at `:2`.

Everything else reaches these surfaces by test id.
`packages/app/stories/space/messaging.stories.tsx:17` and
`packages/app/stories/support/SpaceSidebarFixture.tsx:317` construct failures
with literal messages and will need the structured shape instead, and
`messaging.stories.tsx:38` passes the refusal sentence itself through the
fixture's `remoteRefusal` prop (`SpaceSidebarFixture.tsx:63`, `:264`).
