...
- [ ] The lane module answers lane offset and "connects" per Edge from the Edges and the Active Graph; nothing relies on the order the caller passes them in.
- [ ] The projection no longer reorders Edges to choose the centre lane.
- [ ] The arrowhead and the trim follow "connects"; nothing tests the trim value to decide the arrowhead.
- [ ] Paint order still lifts the Active Graph's Edges, and only those.
- [ ] The canvas draws exactly what it drew before: existing projection, lane and e2e tests pass unchanged apart from calls to the renamed function.
