...
- **The per-function figures come from a parser that mis-spans this repo's TypeScript, in both directions.** ImpactGate takes each unit's line span from lizard (1.23.0 as resolved) and trusts it. On `CommandDock.tsx` at `main`, lizard spans the 9-line branchless `ResourcesTrigger` over lines 1285–2785 and reports CC 92: `export interface DockChrome {` at line 236 derails its state machine a thousand lines before the damage shows. On `space-authoring.ts` after PR #267 it ends `deriveCompletedEdit` at line 1116, where a nested `writeMap` arrow begins, though the function runs 943–1463, and reports CC 30. Any gate would rest on numbers already shown wrong in two of the four largest files.
