...
**Why:** `AggregateInvariantError` was given its own type so two unrelated failures reaching a reader the same way could be told apart (`packages/persistence/src/repository.ts`). Only one of the two got a name. The other is `!isAggregateInvariant(error)` — an else-branch — and four readers now re-derive the same assumption from it, one more than when this ticket was filed: `src/http/space-host.ts:85` re-reads once, `src/http/space-host.ts:151` answers 503, `packages/http/src/index.ts`'s `POST SPACE_COLLECTION_PATH` commit route gained an arm of its own since (`:433`) — `SqlSpaceRepository.commit` (ticket 24) now raises `AggregateInvariantError` the same way a read does, and this route classifies it exactly as the read paths below do — its `GET SPACE_AGGREGATE_PATH` handler still does both for `GET /api/aggregate` (`:454` and `:465`, was `:424`/`:435` before ticket 24's consolidation moved code around them), and `src/startup/database-startup.ts:140` resets its give-up counter and continues.
...
- [ ] Red first: a failure that is neither broken stored state nor a recognised transient one is classified as *neither*, and each of the four readers is asserted at its own seam for that input. The test names the third case; today there is no input that can express it.
...
- [ ] The four readers ask the one predicate rather than re-deriving the arm from `!isAggregateInvariant`: `space-host.ts:85`'s single re-read, `space-host.ts:151`, `packages/http/src/index.ts`'s `POST SPACE_COLLECTION_PATH` commit catch (`:433`) and its `GET SPACE_AGGREGATE_PATH` handler (`:454`/`:465`, moved from `:424`/`:435` since this ticket was filed), and `database-startup.ts:140` all keep their current behaviour for the two named arms.
