...
The CLI is a second process. Ticket 14 showed two independent connections to one live file can stall for the busy timeout even on disjoint rows. **Do not require the CLI and a running host to share a live file.** Operator rule: the host is stopped, or CLI points at a file the host does not have open. A CLI that does hit a held file must fail as exhausted BUSY (retryable), not as a revision conflict. Proving that classification is 18; this ticket only has to not pretend concurrent CLI+host writers are in scope.
