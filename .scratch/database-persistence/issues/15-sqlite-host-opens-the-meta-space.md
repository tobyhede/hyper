...
- [ ] The file lives at a stable application-owned local path. A missing or unwritable parent fails clearly. The runtime is closed on CLI/server shutdown and before test cleanup. Shutdown cancels in-flight Meta establishment retry and awaits it before closing the SQLite adapter, so retry cannot call `establishMetaSpace` after close. A shutdown test covers that order.
