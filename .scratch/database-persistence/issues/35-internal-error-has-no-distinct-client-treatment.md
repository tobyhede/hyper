...
**Scope note.** Not every broken-stored-state commit reaches this mapping as `internal-error`. A stored document that fails its schema on the single-Space fast path is answered 503 `persistence-unavailable` by the server (ticket 31's third case, pinned in `test/support/repository-contract.ts`), so it arrives here already indistinguishable from an outage. As things stand this question covers only the complete-aggregate commit path and unusable stored revisions. If ticket 31 reclassifies the fast-path case, the scope here grows with it.
