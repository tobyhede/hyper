...
**Scope note.** Ticket 31 settled the fast-path case: `#loadStoredSpaceRowForCommit` names a stored document that fails its schema on the single-Space fast path broken stored state (`AggregateInvariantError`), so the server answers it 500 `internal-error` like the complete-aggregate commit path and unusable stored revisions (pinned in `test/support/repository-contract.ts`, "names a broken stored document broken stored state on the single-Space fast path"). This question therefore covers every broken-stored-state commit.
