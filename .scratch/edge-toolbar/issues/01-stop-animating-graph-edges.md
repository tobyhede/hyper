...
Built on `edge-toolbar` (`a0fa5fb2`): `projectGraphEdges` sets `animated: false` with the reason beside it, and `projection.test.ts` asserts no Edge animates, emphasised or not. Nothing else — CSS, stories, the adapter's routed Edge — read `animated`. The solid, still line is the intended look. `pnpm verify` (3103 passed, 13 skipped), `pnpm e2e` (227) and `pnpm e2e:ladle` (115) green, each run on its own.
