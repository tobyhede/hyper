...
- **Gate check.** `thresholds.break: null` (the `thresholds` key in `stryker.conf.mjs` — cited by name because a later commit on this branch moved its line), `grep -rn "mutate\|stryker" .github/workflows/` finds nothing, and `verify` does not call it. All three checked at commit `552f915` and re-checked at branch tip.
