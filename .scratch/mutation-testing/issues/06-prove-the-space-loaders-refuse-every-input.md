...
The largest gap is one rule seen from eight places. `loadSpace(input: unknown, …)` and `loadSpaceSnapshot(input: unknown)` are parsing boundaries, and their production callers are written on the promise that they refuse rather than throw — `parseSnapshot` in `src/persistence/postgres-space-repository.ts:99` turns a refusal into a `SnapshotValidationError`, and `openStoredWorkspace` in `packages/app/src/open-workspace.ts:25` turns one into "The backend returned an invalid space". Nothing proves the promise. Eight mutants across three guard sites — `#7` and `#12`'s neighbour at `space.ts:100:39`, `#27` at `140:39`, `#86`/`#88`/`#89`/`#92` at `250`, and the `'(root)'` message fallbacks `#57`/`#114` at `201`/`260` — survive the **whole 1651-test suite**, each hand-applied and measured. Under them, `loadSpace(null, [])` throws `TypeError: Cannot read properties of null (reading 'version')` (or `Cannot use 'in' operator …`) and `loadSpaceSnapshot(null)` / `loadSpaceSnapshot(undefined)` throw `Cannot read properties of null (reading 'document')`, where all four currently answer `{ ok: false, errors: [{ kind: 'invalid-shape', message: '(root): …' }] }`.
