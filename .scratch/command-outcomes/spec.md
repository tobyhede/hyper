...
- **Dependencies at composition** (ADR 0016's rule — injected once, never per call): `authoring` (for the replacement epoch, which lives on Space Authoring's state rather than Navigation's), `continuation`, `navigation` (for the reset), and `reportObserverError`, from which it builds its own non-throwing `reportBreak`.
...
- **Staleness is the module's, not the caller's** — the rule `resource-deletion.ts` already carries (`interactionEpoch`, and a `disposed` guard on the `.then`), generalised to the table. Each `run` takes, at the press: a per-channel run epoch, the `selectedMapId` for a Map-scoped channel, and the composition's `replacementEpoch`. A settlement publishes only if all three still hold, and is otherwise discarded in silence — `run` answers `COMMAND_DISCARDED`. One exception to the Map check: a `completed` outcome passes it when its **run claims its completion moves the Map** these command outcomes read, because then that completion is what moved the selection; its other outcomes are still held to it. The claim is per run, not per channel: `map-create` and `map-delete` each take a required `completionMovesMap` (`MapCompletionClaim`), so every caller states it. The Dock claims it — its creation selects the new Map on the canvas it stands on, and its deletion leaves that canvas on the survivor. A Space Resource's rail does not — it authors in the target Space while the containing canvas's command outcomes holds the run, and it never moves that canvas, so a rail completion settling after the author changed the containing canvas's Map is discarded like any other stale settlement. A discarded settlement is not a refusal the author asked for (`CONTEXT.md`, Replacement epoch). So an operation begun on Map A cannot draw its notice under Map B, and two runs on one channel cannot land out of order. A discarded settlement still reaches `reportBreak` if it threw: the defect happened whether or not anyone is left to be told. A replacement-epoch move also clears every standing notice, on every channel, since each was about the Space that was replaced — which widens the old `resourceDeletion`'s `discard()`, the one slot that cleared on replacement, to the whole table.
- **`dispose()`** unsubscribes from Navigation and Space Authoring, drops every in-flight run and calls `observable.clearSubscribers()`. It joins the ordered block in `open-spaces.ts:783-788`, before `continuation.dispose()`, since this module publishes into the continuation.
...
|---|---|---|---|
| `map-create` | Map not created | yes | per run: Dock yes, rail no |
| `map-manage` | Map unchanged | yes | no |
| `map-delete` | Map not deleted | yes | per run: Dock yes, rail no |
...
10. `10-discarded-settlements-follow-nothing.md` — `COMMAND_DISCARDED`, the per-run `completionMovesMap` claim, and New Map's continuation through `continueAt`.
