...
- **Review finding G1: the exemption was the channel's, and only the Dock earns it.** `CHANNELS` carried `completionMovesMap` on `map-create` and `map-delete`, so every completion on those channels skipped the Map check. The rail's New Map runs on the containing canvas's command outcomes, which capture that canvas's Map, yet it authors in the target Space and never moves the containing canvas; the embedded adapter waits in `before` and again in `after` for both Spaces to save and for the selection write. An author who switched the containing canvas to another Map in that window still got the completion through, and its `map-name` continuation — which `ChromeContinuation` holds rather than drops when the rail is not drawn — later opened a rename nobody asked for on returning to the first Map. The rail's Delete Map had the same exemption, harmlessly (a completed deletion publishes nothing and requests no continuation). **Fix:** the claim moved from the channel to the run. `map-create`'s options are `MapCreateContinuation & MapCompletionClaim` and `map-delete`'s are `[MapCompletionClaim]`, both with `completionMovesMap` required and no default, so the compiler makes every caller state it; the Dock passes `true` for both, the rail `false` for both, and `CHANNELS` no longer carries the flag. The definition's `completed` predicate became `movedMap(result, ...options)`, which answers `completed && completionMovesMap`. `command-outcomes.test.ts` now holds that a creation and a deletion run the rail's way, completing after the containing canvas changed Map, answer `COMMAND_DISCARDED` and request no continuation; the Dock's form keeps its "completion moves the Map" tests. `map-authoring-commands.test.ts`'s contract contexts carry each surface's claim.
