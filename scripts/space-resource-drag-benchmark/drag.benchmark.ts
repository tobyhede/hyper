...
const configFile = fileURLToPath(
  new URL('../../packages/app/e2e/space-resource-drag-benchmark-vite.config.ts', import.meta.url),
);
...
async function dragAndMeasure(
  ...
  await page.evaluate(() => performance.clearMarks('hyper:embedded-map-publication'));
  ...
  const embeddedPublications = await page.evaluate(
    () => performance.getEntriesByName('hyper:embedded-map-publication').length,
  );
...
test('records repeatable Space Resource drag diagnostics', async ({ page }, testInfo) => {
  ...
  const ordinary = page.locator('.react-flow__node').filter({
    has: page.getByRole('heading', { name: 'Ordinary Markdown Resource', exact: true }),
  });
  ...
  const counts = {
    mountedResources: await page.locator('.react-flow__node').count(),
    mountedEdges: await page.locator('.react-flow__edge').count(),
    visibleResources: await page.locator('.react-flow__node:visible').count(),
