...
- `BENCHMARK_SCALE`: `10`, `50`, `100`, or `500` embedded Markdown Resources
- `BENCHMARK_DENSITY`: `sparse` or `dense`
- `BENCHMARK_OPEN_PARENTS`: `1` or `3`

Every scenario includes an ordinary Markdown Resource, the selected number of Open Space Resources and a second Space Resource embedding level. The generator validates the complete aggregate through normal intake. The browser asserts exact mounted and visible Resource and Edge counts before measuring.

Generate a scenario from the repository root:

```sh
BENCHMARK_SCALE=10 BENCHMARK_DENSITY=sparse BENCHMARK_OPEN_PARENTS=1 \
  pnpm exec tsx scripts/space-resource-drag-benchmark/generate.ts
```
...
BENCHMARK_MOVEMENT_CORRECTION=present BENCHMARK_BUILD_MODE=development \
  pnpm exec playwright test --config scripts/space-resource-drag-benchmark/playwright.config.ts
```
...
The runner performs the same 161px gesture on the first Space Resource, its first embedded Resource and the ordinary Markdown Resource. During the parent gesture it samples every visible embedded Resource and Edge against the moving parent, reporting follower and connector drift independently. It also records actual embedded-publication counts, frame intervals, observed long tasks, React commit-hook counts, DOM mutation records, Chrome performance-counter deltas for scripting, layout and total task time, and paint duration from a retained DevTools timeline. Geometry sampling, observers, tracing, the commit hook and benchmark instrumentation all add overhead.

## Recorded smoke baseline

Recorded on 2026-09-14 from the `10-sparse-1` scenario on an Apple M2 with eight logical CPUs, Chromium 149, a 1440×1000 viewport and the movement correction present. The scene contained 27 mounted/14 visible Resources and 18 mounted/9 visible Edges.
