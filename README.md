...
`SQLITE_PATH` names the file. Use a stable absolute path in a local directory the application owns; the host and CLI refuse to start when it is unset or blank, or when its parent directory is missing or not writable, and both also refuse a relative path — `pnpm db:migrate:sqlite` runs from the repository root while the host runs from `packages/app`, so a relative value would migrate one file and open another. `pnpm dev:sqlite` checks for that before migrating, refusing a relative `SQLITE_PATH` before anything under the repository root is touched; the host checks again at composition, so a relative path reaching it by another route is still refused. A network filesystem, a file shared between hosts and a hosted SQLite service are not supported deployments.
