...
`SQLITE_PATH` names the file. Use a stable absolute path in a local directory the application owns; the host and CLI refuse to start when it is unset or blank, or when its parent directory is missing or not writable, and the host also refuses a relative path, which `pnpm dev:sqlite` would otherwise migrate from the repository root and open from `packages/app`. A network filesystem, a file shared between hosts and a hosted SQLite service are not supported deployments.
