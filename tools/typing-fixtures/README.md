...
`mismatched-menu-item.tsx` is the one fixture about a **seam** rather than a repository-wide rule, and the one that imports application source. `DropdownMenuRadioGroup` absorbs Base UI's `any` so that no consumer launders the value handed back to it, and the Command Dock's menus gave up their runtime re-parsing on the strength of that — a promise worth exactly the compiler error standing behind it, which is what this holds in place. It reaches the component by relative path rather than through `@project/ui`, because the barrel drags five side-effect CSS imports into a program with no `vite/client` types. It is also the reason the `tsc` diagnostic reader in `test/unit/typing-fixtures.test.ts` matches `.tsx?` and not `.ts`.
